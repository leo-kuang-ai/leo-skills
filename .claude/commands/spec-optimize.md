...
Command support root: `.claude/spec-first/workflows/spec-optimize`. Treat it as the loaded skill directory whenever this inlined workflow refers to `SKILL_DIR` or the directory containing `SKILL.md`.
...
Use `.claude/spec-first/workflows/spec-optimize/scripts/decide.cjs` for numeric multi-objective eligibility, comparison thresholds, and measurement-ladder steps. A required-objective win can be kept even when the primary is unchanged; confirmation and integration must finish before recording `kept`. Completion requires every declared required target or another explicit stop, a verified final log, and an honest account of blocked or unfinished work. `.claude/spec-first/workflows/spec-optimize/references/example-expensive-benchmark-spec.yaml` illustrates multiple required targets and staged sampling.
...
`.claude/spec-first/workflows/spec-optimize/references/optimize-spec-schema.yaml`

## Experiment Log Schema

Reference the experiment log schema for state management:

`.claude/spec-first/workflows/spec-optimize/references/experiment-log-schema.yaml`

## Quick Start

For a first run, optimize for signal and safety, not maximum throughput:

- Start from `.claude/spec-first/workflows/spec-optimize/references/example-hard-spec.yaml` when the metric is objective and cheap to measure
- Use `.claude/spec-first/workflows/spec-optimize/references/example-judge-spec.yaml` only when actual quality requires semantic judgment
...
`.claude/spec-first/workflows/spec-optimize/references/usage-guide.md`
...
`.claude/spec-first/workflows/spec-optimize/references/measurement-only-calibration.md`. This mode compares an explicitly
...
`node .claude/spec-first/workflows/spec-optimize/scripts/measurement-admission.cjs admit --input <path>`. Persist the
...
- Phase 0 spec input and validation: `.claude/spec-first/workflows/spec-optimize/references/spec.md`
- Phase 0.3-1.7 measurement scaffolding and approval gate: `.claude/spec-first/workflows/spec-optimize/references/measurement.md`
- Persistence, checkpoints, and resume: `.claude/spec-first/workflows/spec-optimize/references/persistence.md`
- Phases 2-3 hypotheses and optimization loop: `.claude/spec-first/workflows/spec-optimize/references/loop.md`
- Phase 4 deferred work, summary, and cleanup: `.claude/spec-first/workflows/spec-optimize/references/wrap-up.md`
...
1. Read `.claude/spec-first/workflows/spec-optimize/references/persistence.md` before Phase 0 or any run-state write. Checkpoints CP-0 through CP-5 require write-then-read verification; a checkpoint proves persisted work, never a user decision.
2. Read `.claude/spec-first/workflows/spec-optimize/references/spec.md` and save or validate the optimization spec. On resume, first detect the existing run and recover markers; do not overwrite its spec or baseline.
3. Read `.claude/spec-first/workflows/spec-optimize/references/measurement.md` and clear measurement authorization, baseline, and approval gates.
4. Read `.claude/spec-first/workflows/spec-optimize/references/loop.md` for bounded hypothesis and experiment execution.
5. Read `.claude/spec-first/workflows/spec-optimize/references/wrap-up.md` to close with evidence, deferred items, and durable local state.

On resume, skip only work that current disk evidence proves complete. Re-enter any gate whose clearance is absent from visible user or upstream authorization; do not ask again when that evidence already covers the unchanged scope. An iteration number alone cannot prove baseline approval. See `.claude/spec-first/workflows/spec-optimize/references/persistence.md` for recovery and `.claude/spec-first/workflows/spec-optimize/references/measurement.md` for spec adjustment before derived state exists.

CP-5 finalizes the log only when the selected next step does not return to Phase 3. Continue or approved deferred dependencies leave the run resumable; `.claude/spec-first/workflows/spec-optimize/references/wrap-up.md` owns that decision and cleanup.
