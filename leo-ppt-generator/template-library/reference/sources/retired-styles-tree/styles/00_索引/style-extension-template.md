...
> 库规模以[生成计数](../generated/counts.md)为准。用户侧扩展通道（AE-39）：手工新增风格**统一走本模板 +
...
4. **登记**：维护者重新生成 style-index 并运行只读检查，不手改索引行或 counts；
   配对关系由现有 `视觉风格配对.md` 合同处理。source/taxonomy 可选字段存在即校验，
   来源未核验时 license=unknown，不从所在目录推断许可；未富化字段保留 absent。

## 一、风格定义

- 风格名称：`<风格名，中文，以"风"结尾；先查名称索引避免重名，共享别名须保留消歧入口>`
