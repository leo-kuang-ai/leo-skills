...
裸主题指请求只给出主题，没有 reader job、体裁信号、目标读者、现有 brief 或交付目标。处理按信号分两级，问询是例外而不是默认：

**有信号 → 声明假设并继续。** 请求含以下任一信号时不再问询：体裁词（如「深度分析」「介绍」「教程」「复盘」「报告」）、读者词（如「面向工程师」「给管理层看」）、渠道词（公众号、知乎、Newsletter）、用途词（如「要发布」「内部分享」）。声明一句有边界的假设（family + 读者推断），随即进入对应 workflow；用户可随时在产物上纠正。

**无信号且候选 family 分叉 → 只问一个问题。** 只有当两个候选 family 会产生不同的证据门禁和交付物（如观点评论 vs 教程）时，才使用问询：
...
不要一次询问体裁、字数、平台、语气、读者和材料。先解决 workflow 分叉，进入对应 brief 后再补真正缺失的字段。拿不准时优先声明假设并继续——让用户在产物上纠正，比停在问卷上更省双方时间。

## 路由卡

内部始终生成，默认不展示。只有用户要求查看计划、或存在会改变交付物形态的假设需要显式确认时才展示；chat 快速交付可省略展示，假设改为一行带过。
