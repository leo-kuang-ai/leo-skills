...
- 默认不覆盖源文件；用户显式要求写回时，先建立快照并如实披露已按指令写回（建议保留备份或写新文件）。不静默修正原文事实；输入不完整时报告受限状态。
- 原文歧义必须保留并披露：分析阶段登记歧义、疑似错误（如与产品默认值、常识或上下文矛盾的端口、版本与数字）与未定义术语，交付时列入"歧义与原文问题清单"，不得为读起来顺而替作者选定解释。
...
- 翻译准确性与中文编辑质量分开判断；默认交付为 `publication` 双层模式（保真初译 → 受约束编辑润色 → 逐句对照审校 → 五件套发布输出），其合同常量为 `fidelity_priority: strict`、`editorial_polish: constrained`、`source_correction: prohibited`、`ambiguity_handling: preserve_and_disclose`、`technical_review: required`；`faithful`（仅保真直译）与 `polished`（轻润色）为显式降档。发布判定为合取：保真失败或编辑越界即整体不可发布，中文流畅度不可抵消。publication 的五件套节必须在场（译文正文、翻译说明、技术术语表、歧义与原文问题清单、未解决事项与需作者确认；短文同样适用，无已知项写"无已知…"，不得省略节或虚构内容）。
...
2. 阶段 1（源文分析）：进行全篇分析（文章类型、主题/受众/语气/术语/隐喻/结构难点），标记事实、观点、假设、警告、限制、因果、条件、否定与模态，并登记原文歧义、疑似错误与未定义术语（并入会话挑战清单）；识别保护区和高风险语句；`analysis_depth: skip` 仅适用于短、低风险的 `faithful` 任务。
3. 阶段 2（术语处理）：加载内置英中术语表 `references/glossary-en-zh.md`，再合并用户术语表和项目术语表；形成会话级术语与挑战清单。同一术语按 sense 和 scope 统一，隐喻记录"保留/转写/解释"的选择。`publication` 模式下术语首次出现采用"中文译名（英文原文）"形态；API、类名、命令、配置键、协议名与代码标识保留原文，不因中文习惯替换技术概念。
4. 阶段 3（保真初译）：先生成保真初译并完成语义复核；不得在此阶段引入编辑性改写。
5. 阶段 4（中文技术编辑，仅在 `polished` / `publication` 模式执行，默认即含）：清除英文句法、调整中文节奏、上下文化产品机制术语、优化标题和段落，但不得新增事实、例子、因果、承诺或结论，不得把 may/can/could/might 强化为确定性表达，不得把建议改成要求、限制改成保证，不得改变责任主体或削弱技术约束。
...
7. 阶段 5（独立保真审校）：对润色后译文逐句对照原文，按七类问题记录——漏译、误译、语义强化、语义弱化、术语不一致、结构损坏、编辑越界，覆盖语义、模态、因果、条件、量词、风险与责任主体；分别记录 `technical_review` 与 `editorial_review`，编辑通过不抵消保真失败；未做独立复核不得标记为 `independent_review`。
8. 阶段 6（发布前输出）：`publication` 模式交付五件套——译文正文、简短翻译说明、技术术语表、歧义与原文问题清单、未解决事项与需作者确认事项；无已知项的节可写"无已知…"，但节必须在场且不得虚构内容，长文任务不得以篇幅为由省略五件套（最终消息在译文正文之后依次附上）。`faithful` / `polished` 沿用轻量输出（译文、交付模式、覆盖范围、保护检查、审校状态、未解决项和限制）。
