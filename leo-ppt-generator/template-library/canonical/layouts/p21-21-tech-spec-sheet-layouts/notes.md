...
绑定模板：`builtin:template:table-pro`。页面角色：`data`。

阅读顺序：`kicker` → `title` → `columns` → `rows` → `footnote`。
...
- `columns`：区域 `table_zone`，2–5 项。
- `rows`：区域 `table_zone`，1–6 项。
