...
The full 16-step subprocess pipeline, runner-owned fail-fast reason codes, the auto-stub path for unstaged issues, and the failure-envelope behavior are documented in [Headless Runner And Artifact Lifecycle](.claude/spec-first/workflows/spec-app-consistency-audit/references/headless-runner.md). Current focused tests cover source/runtime host boundaries; runner scripts remain directly syntax-checked and exercised through the repository test suites.
...
```text
.claude/spec-first/workflows/spec-app-consistency-audit/prompts/
```
...
Full command detail lives in [Mode, Output, And Issue Contract](.claude/spec-first/workflows/spec-app-consistency-audit/references/mode-output-contract.md).
...
Detailed field rules are in [Mode, Output, And Issue Contract](.claude/spec-first/workflows/spec-app-consistency-audit/references/mode-output-contract.md).
...
```text
.claude/spec-first/workflows/spec-app-consistency-audit/scripts/
```
...
```text
.claude/spec-first/workflows/spec-app-consistency-audit/rule-packs/
```
...
- [Headless Runner And Artifact Lifecycle](.claude/spec-first/workflows/spec-app-consistency-audit/references/headless-runner.md): full runner pipeline, fail-fast reason codes, and artifact-lifecycle/enum rules deferred from this entry.
- [Mode, Output, And Issue Contract](.claude/spec-first/workflows/spec-app-consistency-audit/references/mode-output-contract.md): detailed mode tokens, scope resolution, Figma materialization, output/writeback, and issue protocol fields.
- [`.claude/spec-first/workflows/spec-app-consistency-audit/references/report-format.md`](.claude/spec-first/workflows/spec-app-consistency-audit/references/report-format.md): the report section structure that downstream Report-Writer steps emit.
- [`.claude/spec-first/workflows/spec-app-consistency-audit/references/pilot-validation.md`](.claude/spec-first/workflows/spec-app-consistency-audit/references/pilot-validation.md): minimal pilot-record format gating the v0.2 readiness decision.
- [`.claude/spec-first/workflows/spec-app-consistency-audit/references/ecc-source-lock.json`](.claude/spec-first/workflows/spec-app-consistency-audit/references/ecc-source-lock.json): ECC source-lock policy consumed by `.claude/spec-first/workflows/spec-app-consistency-audit/scripts/merge-contracts.js` and the orchestrator prompt to keep ECC-derived lenses read-only.
