...
| 类别 | 数量 | 说明 |
|---|---|---|
| **可加载风格**（含 JSON brief） | 273 份 | 11 顶层内置 + 262 子目录参考（262 = 150+69+40+3，不含 `04_来源_guizang/` 的 7 份组件模板）；`leo-ppt style list` 递归枚举全部 273 个。R-66 家族合并：其中 17 份同板场景变体经 `variant_of` 归并至 8 个主风格家族（独立可选风格 256 个），变体原名保留、经主风格 `aliases` 检索 |
...
- 视觉风格配对：`00_索引/视觉风格配对.md`（视觉风格 ↔ 图片渲染逐条配对，273 组全覆盖：每个可加载风格都有渲染锚）
...
- `01_通用母版/`（110）视觉风格 · `02_行业内容域/`（59）行业身份 ·
...
`00_索引/` 另有 9 份规则/真值文档：`_INDEX`、`设计体系`、`通用设计规范`、
