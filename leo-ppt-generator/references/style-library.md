...
执行期风格身份只以 [`template-library/catalog/current.json`](../template-library/catalog/current.json)
指向的 generation `registry.json` 为准：过滤 `kind=style` 的实体读取 name、aliases、path、
revision 和 lifecycle。风格数量与名称/别名命中均从这份 registry 或同一 home 的
`style list --summary --filter` 结果派生，不读取退役的 `generated/` 快照。
...
- [设计体系](../template-library/governance/authoring/index/设计体系.md)：多轴组合原则。
- [视觉风格配对](../template-library/governance/authoring/index/视觉风格配对.md)：随包风格的既有配对；不能因 user 同名就继承。
- [通用设计规范](../template-library/governance/authoring/index/通用设计规范.md)：上位护栏。
- [版式调度](layout-dispatch.md)：角色、容量、节奏与独立预检；无用户专属配置时仅声明通用布局。
- [扩展模板](../template-library/governance/authoring/index/style-extension-template.md)：新增/修订 brief 的治理流程。
...
索引缺失或损坏时，resolver 按 SKILL.md 走 canonical 的只读重建/降级；不读取退役的
`generated/` 快照。advise 不执行工具，按 SKILL.md 的固定降级说明结束本次目录查询。
...
不进生图链路；`template-library/governance/authoring/index/` 下的通用设计规范/设计体系约束 agent 行为、无注入路径。
...
纪律与忌配表见 [`../template-library/governance/authoring/index/设计体系.md`](../template-library/governance/authoring/index/设计体系.md) §3a/§3b。
...
- 两张参考图只是比较或融合的视觉输入，不等于授权生成两张付费样张；可提议双生，
  但须先披露额外图片成本并取得授权，且双生遵守样张门。

## 保存自定义风格

用户要求保存当前风格时，先检查真实最终页面，至少覆盖封面、普通内容、复杂机制或
数据页、结尾页。只提取可复用视觉系统，不保存业务正文、个人信息、客户数据或必须
依赖的私有图片。自定义风格保存到 `${LEO_PPT_HOME}/template-library/canonical/styles/<slug>/brief.json`，不得
