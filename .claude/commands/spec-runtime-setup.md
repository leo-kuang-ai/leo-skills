...
Command support root: `.claude/spec-first/workflows/spec-runtime-setup`. Treat it as the loaded skill directory whenever this inlined workflow refers to `SKILL_DIR` or the directory containing `SKILL.md`.
...
`.claude/spec-first/workflows/spec-runtime-setup/scripts/check-health` 是带 Node shebang 的 compatibility shim，委托给 `setup.cjs --check`。Windows 直接调用 `node <loaded-skill-root>/scripts/setup.cjs --check`，不存在 platform-specific companion entry。
...
Before any project-local write, read `.claude/spec-first/workflows/spec-runtime-setup/references/project-config.md`. Project-local config actions never install providers or edit host config; host/provider actions never migrate local config keys.
