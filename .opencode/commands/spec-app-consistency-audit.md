...
Command support root: `.opencode/skills/spec-app-consistency-audit`. Treat it as the loaded skill directory whenever this inlined workflow refers to `SKILL_DIR` or the directory containing `SKILL.md`.
...
v1 deterministic orchestrator (`.opencode/skills/spec-app-consistency-audit/scripts/run-audit.js`) accepts `mode:headless` only; `mode:default` and `mode:report-only` orchestration is deferred. Long-lived canonical-token semantics live in [Mode, Output, And Issue Contract](.opencode/skills/spec-app-consistency-audit/references/mode-output-contract.md).
...
`latest-summary.json` is only a pointer; consumers must validate it against `metadata.json`. Artifact status, `issue_synthesis_status`, and the full spine are documented in [Headless Runner And Artifact Lifecycle](.opencode/skills/spec-app-consistency-audit/references/headless-runner.md) and [Mode, Output, And Issue Contract](.opencode/skills/spec-app-consistency-audit/references/mode-output-contract.md).
...
`.opencode/skills/spec-app-consistency-audit/scripts/run-audit.js` is the deterministic entrypoint for the static artifact chain. It is a subprocess orchestrator only — it never invents issues, never calls an LLM, and never fetches remote Figma/PRD assets. The runner accepts `mode:headless` only and requires `base:<git-ref>`; missing base returns `scope_headless_missing_base`.

The full 16-step subprocess pipeline, runner-owned fail-fast reason codes, the auto-stub path for unstaged issues, and the failure-envelope behavior are documented in [Headless Runner And Artifact Lifecycle](.opencode/skills/spec-app-consistency-audit/references/headless-runner.md). Current focused tests cover source/runtime host boundaries; runner scripts remain directly syntax-checked and exercised through the repository test suites.
...
```text
.opencode/skills/spec-app-consistency-audit/prompts/
```
...
Full command detail lives in [Mode, Output, And Issue Contract](.opencode/skills/spec-app-consistency-audit/references/mode-output-contract.md).
...
Detailed field rules are in [Mode, Output, And Issue Contract](.opencode/skills/spec-app-consistency-audit/references/mode-output-contract.md).
...
```text
.opencode/skills/spec-app-consistency-audit/scripts/
```
...
```text
.opencode/skills/spec-app-consistency-audit/rule-packs/
```
...
- [Headless Runner And Artifact Lifecycle](.opencode/skills/spec-app-consistency-audit/references/headless-runner.md): full runner pipeline, fail-fast reason codes, and artifact-lifecycle/enum rules deferred from this entry.
- [Mode, Output, And Issue Contract](.opencode/skills/spec-app-consistency-audit/references/mode-output-contract.md): detailed mode tokens, scope resolution, Figma materialization, output/writeback, and issue protocol fields.
- [`.opencode/skills/spec-app-consistency-audit/references/report-format.md`](.opencode/skills/spec-app-consistency-audit/references/report-format.md): the report section structure that downstream Report-Writer steps emit.
- [`.opencode/skills/spec-app-consistency-audit/references/pilot-validation.md`](.opencode/skills/spec-app-consistency-audit/references/pilot-validation.md): minimal pilot-record format gating the v0.2 readiness decision.
- [`.opencode/skills/spec-app-consistency-audit/references/ecc-source-lock.json`](.opencode/skills/spec-app-consistency-audit/references/ecc-source-lock.json): ECC source-lock policy consumed by `.opencode/skills/spec-app-consistency-audit/scripts/merge-contracts.js` and the orchestrator prompt to keep ECC-derived lenses read-only.
