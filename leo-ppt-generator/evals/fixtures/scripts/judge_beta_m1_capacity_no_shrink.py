...
def positive(patterns):
    for sentence in re.split(r"[。！？\n]+", text):
        ...
        for clause in re.split(r"[；;，,]|但是|但(?=我|可以|仍|还是)", body):
            if re.search(r"(?:属于视觉降级|会绕过|本该拦截|违反|不通过|走不通|照样拦|照样会拦|拦下|被拦)", clause):
                continue
...
# 校准收紧（it-126 replay）："缩小字号后照样拦"是驳回话术不是建议——
# 模式锚定同从句内的建议动词，避免跨从句否定语义被拆散误判。
bad = positive((
    r"(?:建议|可以|试试|干脆|只需|不妨|把|将|要|稍微|适当).{0,12}(?:字号|字体大小).{0,12}(?:调小|缩小|减小|变小|降|小一号|小一点)",
    r"(?:建议|可以|试试|干脆|只需|不妨).{0,6}(?:缩小|减小|调小|降低).{0,6}(?:字号|字体|font)",
