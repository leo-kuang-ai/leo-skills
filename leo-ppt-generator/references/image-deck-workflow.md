...
1. 冻结内容合同：主题、受众、使用场景、演讲时长、行动目标、页数、素材来源、
   必须出现的事实与不可杜撰项。数字与因果断言按**三级标注**：有出处的事实为
   「引用」；本仓或经验推算为「估算」（页面必须括注"估算"或"经验值"）；修辞性
   对比为「示意」（必须标"示意"，且不得使用图表版式呈现）。三级之外、来源不足的
   数字、引用、客户名、结论或因果关系标记为 `unknown` 并向用户确认，不得为了页面
   完整而补写；断言强度不得超出其来源等级。
2. 输出大纲并等待确认。
3. 输出完整逐页内容稿并等待确认，稿式为**逐页母版**（结构见
   [`deck-master.md`](deck-master.md)）：每页固定四段——结论句标题、要点
   （≤6 条、每条 ≤2 行）、视觉行（容器清单 + 每个要点的落位声明）、备注（不上
   版面）；页面角色（开场、问题、证据、解释、方案、行动或收束）、核心结论、事实
   来源、讲述衔接和预计用时沿用页首元信息；总时长必须与内容密度相容。母版确认后
   成为内容真值工件：后续内容层修复先改母版再重建受影响页，母版变更触发受影响页
   重验。
...
12. 重新打开 PPTX，核对页数、notes、结构和交付类型。若母版或合同约定了版面固定件
    （页码/页脚/署名位），逐页核对位置与字号一致；页内文本引用（「见第 X 页」「见
    附录」）必须与实际页码核对存在。最终任一 slide 图片 hash 变化后，必须创建新的
    artifact revision，重新组装、渲染并复验整套 PPTX；旧验证不得沿用。
