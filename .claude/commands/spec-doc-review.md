...
Command support root: `.claude/spec-first/workflows/spec-doc-review`. Treat it as the loaded skill directory whenever this inlined workflow refers to `SKILL_DIR` or the directory containing `SKILL.md`.
...
**STOP. 当 classification 为 `task-pack` 时，立即读取 `.claude/spec-first/workflows/spec-doc-review/references/task-pack-review-lens.md`，完成 deterministic intake、current source-plan read、task-pack-specific semantic lens 与 terminal-owner mapping；这些步骤必须在 persona selection/dispatch 前完成。**
...
**STOP. If classification is genuinely ambiguous after applying the core rules above, read `.claude/spec-first/workflows/spec-doc-review/references/document-classification-signals.md` for the full signal lists before proceeding to persona selection.**
...
**STOP. If the quick-reference table does not resolve whether to activate a conditional persona for this document, read `.claude/spec-first/workflows/spec-doc-review/references/persona-activation-matrix.md` before finalizing the reviewer list.**
...
For each selected reviewer, read the matching skill-local prompt asset at `.claude/spec-first/workflows/spec-doc-review/references/personas/<reviewer-name>.md` and pass its full content as `{persona_file}`. Do not dispatch standalone agents by type/name or rely on platform-level custom-agent registration.
...
Before rendering any finding, read `.claude/spec-first/workflows/spec-doc-review/references/rendering-floor.md`. Its consequence-first wording, recommendation visibility, opaque-token budget, and trace-on-request rules apply to the structured envelope, batch report, walkthrough, bulk preview, and persisted Open Questions entry. Surface-specific layouts may differ, but none may weaken that shared decision floor.

After all dispatched agents return, read `.claude/spec-first/workflows/spec-doc-review/references/synthesis-and-presentation.md` for the synthesis pipeline (validate, anchor-based gate, dedup, cross-persona promotion, contradiction resolution, auto-promotion, three-tier routing with FYI subsection), mutation-policy enforcement, structured envelope output, and the routing-question handoff.

Only when `delivery_mode: interactive` **and** `mutation_policy: markdown-write`, read `.claude/spec-first/workflows/spec-doc-review/references/walkthrough.md` for the four-option routing question and per-finding walk-through. For the bulk-action preview used by best-judgment routing, Append-to-Open-Questions, and walk-through's "Auto-resolve with best judgment on the rest", read `.claude/spec-first/workflows/spec-doc-review/references/bulk-preview.md`. Do not load either before agent dispatch completes, and never load them for `report-only`.
...
仅当 Phase 1 分类为 `task-pack` 时读取 [Task Pack Review Lens](.claude/spec-first/workflows/spec-doc-review/references/task-pack-review-lens.md)。

### Subagent Template

@./.claude/spec-first/workflows/spec-doc-review/references/subagent-template.md

### Findings Schema

@./.claude/spec-first/workflows/spec-doc-review/references/findings-schema.json

Selected reviewer prompt assets live under `.claude/spec-first/workflows/spec-doc-review/references/personas/`. Read only the prompt files selected for the current review.
