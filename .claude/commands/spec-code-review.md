...
Command support root: `.claude/spec-first/workflows/spec-code-review`. Treat it as the loaded skill directory whenever this inlined workflow refers to `SKILL_DIR` or the directory containing `SKILL.md`.
...
14 reviewer personas in layered conditionals, plus spec-first local prompt assets. Quick roster with one-line triggers below; the persona catalog in `.claude/spec-first/workflows/spec-code-review/references/persona-catalog.md` (read it at Stage 3) has the full per-persona selection criteria and spawn gates. Each selected reviewer is a generic subagent seeded with a local prompt file from `.claude/spec-first/workflows/spec-code-review/references/personas/`; do not dispatch standalone agents by type/name.
...
**spec-first always-on local prompt assets** (`agent-native-reviewer`, `learnings-researcher`) are dispatched as generic subagents through the same bounded parallel scheduler as the structured personas. Read their prompt files from `.claude/spec-first/workflows/spec-code-review/references/personas/`, then give them the same review context bundle the personas receive: entry mode, any PR metadata gathered in Stage 1, intent summary, review base branch name when known, `BASE:` marker, file list, diff, and `UNTRACKED:` scope notes. Do not invoke them with a generic "review this" prompt. Their output is unstructured and synthesized separately in Stage 6.

**spec-first conditional local prompt assets** (`deployment-verification-agent` only) are dispatched as generic subagents through the same bounded parallel scheduler only when Stage 3 already selected the asset after both the migration/schema-artifact gate and risky-change gate passed. A migration artifact alone, including a safe additive migration, does not authorize Stage 4 dispatch. Read the prompt file from `.claude/spec-first/workflows/spec-code-review/references/personas/`, then pass the same review context bundle plus the Stage 3 applicability reason naming at least one artifact path and the concrete risky operation (for example, a backfill or a NOT NULL change without a default). Its output is unstructured and must be preserved for Stage 6 synthesis just like the spec-first always-on local prompt assets. Schema drift is handled by the `data-migration` persona as structured findings — not here.

#### Cross-model adversarial pass

When `adversarial-reviewer` was selected (Stage 3) **and** scope is `local-aligned` or standalone, load `.claude/spec-first/workflows/spec-code-review/references/cross-model-review.md` and evaluate its admission gates. The peer lifecycle is optional and non-blocking for the main review, but its start is fail-closed: no canonical authorization journey receipt, mismatched receipt/request/payload hashes, non-allowlisted `input_refs`, incomplete external-data authorization, failed redaction, same-provider selection, or requested/actual model mismatch means no peer process and no independent coverage claim.
...
Independent verification gate. Spawn one validator sub-agent per surviving finding using `.claude/spec-first/workflows/spec-code-review/references/validator-template.md`. Findings the validator rejects are dropped; confirmed findings flow through unchanged.
...
**Before writing, load `.claude/spec-first/workflows/spec-code-review/references/review-output-template.md` and mirror its section skeleton** — that file is the canonical skeleton for *which sections appear and in what order*; its example shows one good rendering, not the only permitted layout. The direction below is the always-loaded fallback so it survives a long session even if the template was not reloaded.
...
|-----------|---------|---------|
| `.claude/spec-first/workflows/spec-code-review/references/persona-catalog.md` | Stage 3 | Full per-persona selection criteria and spawn gates |
| `.claude/spec-first/workflows/spec-code-review/references/subagent-template.md` | Stage 4 | Dispatch shape for every persona subagent |
| `.claude/spec-first/workflows/spec-code-review/references/diff-scope.md` | Stage 4 | Shared diff-scope rules passed to each subagent |
| `.claude/spec-first/workflows/spec-code-review/references/findings-schema.json` | Stage 4 | JSON output contract passed to each subagent |
| `.claude/spec-first/workflows/spec-code-review/references/cross-model-review.md` | Stage 4 (only when the cross-model adversarial pass runs) | Host self-identification + peer-CLI shell-out |
| `.claude/spec-first/workflows/spec-code-review/references/action-class-rubric.md` | Action Routing (as needed) | Persona guidance for `autofix_class` |
| `.claude/spec-first/workflows/spec-code-review/references/apply-findings.md` | Stage 5c (admitted default apply-only runs) | Apply, verification, and commit boundaries kept out of report-only context |
| `.claude/spec-first/workflows/spec-code-review/references/review-output-template.md` | Stage 6 | Canonical section skeleton for the report |

Selected reviewer prompt assets live under `.claude/spec-first/workflows/spec-code-review/references/personas/`. Read only the prompt files selected for the current review.
