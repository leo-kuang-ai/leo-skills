...
- **leo-ppt-generator：接入正式 U10 候选视图**：新增 execution-candidate-view schema 与判别式摘要校验，hard-qualified pool 必须与派生视图中的完整身份一致；逐 relation/lane 证据摘要独立，视图显式区分 publication/validation 用途。 (user-visible)
- **leo-ppt-generator：将表达链真实页面纳入交付收据**：HTML/image 产物位置由 pipeline owner 派生，收据覆盖真实页图、对应 sidecar 和当前预览；缺页不能创建完整收据，摘要漂移按稳定 page_id 映射回单页。双 lane 的物化摘要与设计摘要分别保留，不再只取主 lane。 (user-visible)
- **leo-ppt-generator：冻结并重验 U2/U4 资格证据闭包**：原子输入纳入所用 relation/lane 的 receipt、oracle、原始观测与真实产物字节，消费端重新验证资格；证据缺失、漂移或声明篡改不再靠资格字段放行。事务测试改为当前环境的真实浏览器正反探针，继续保留非发布用途；缺 lane 依赖的候选带原因排除。 (user-visible)
- **leo-ppt-generator：补齐独立事项与因果页的角色视图**：在 v2 regime 增加“并列·事项”“因果·论证”到 content layout 的正式映射，使显式表达能通过角色检查，无需借用其他叙事角色。 (user-visible)
- **leo-ppt-generator：区分 U4 非发布验证用途**：PipelineRequest、冻结输入与 materialization binding 显式绑定 publication/validation；只有实际正反证据通过且仅缺视觉裁决的 provisional 可进入非发布验证，unverified 仍拒绝，结果不能获得 publication_ready。 (user-visible)
- **leo-ppt-generator：按完整身份合并 U10 派生配对**：同执行身份的多个已验证关系归入一条候选，重复派生去重；槽位资格复用正式 input_path 合同，保留缺依赖与缺槽位原因。该视图尚待全部生产消费者接入。 (user-visible)
- **leo-ppt-generator：拒绝跨 lane 不一致的 impact 输入**：页级失效计算先验证同一 snapshot 的 HTML/image 表达摘要一致，防止迭代顺序覆盖不同内容后输出虚假的最小失效集合。 (user-visible)
- **leo-ppt-generator：补上 U2 真实输出 oracle**：HTML 渲染可保存同次截图的文字矩形、SVG 连线、画布与溢出测量；资格 verifier 从独立用例重新计算关系检查，拒绝只有布尔 passed 的观测。环境指纹补充浏览器二进制、离线字体和 vendored 依赖字节；image 尚无独立输出检测时明确拒绝晋升。 (user-visible)
- **leo-ppt-generator：让 U6 scorecard 与 impact 使用当前字节证据**：移除文件存在、观测事件即质量通过的判定；四通道重新验证已提交输入、事实与单位投影、绑定和真实 PNG/收据对应，HTML、image、视觉状态独立。impact-v2 直接比较实际双层绑定，拒绝手工 changed 标记；缺真实回放继续 blocked/not_run。 (user-visible)
- **leo-ppt-generator：限定 U5 task-local proposal 操作与作用域**：移除旧 op 兼容入口；按 base 摘要、generation、run scope、lane、anchor 和槽位类型校验四种封闭操作，真实应用几何变更并拒绝越界、自由 CSS、内容丢失和身份冲突。最多三个候选，curation 仅记录证据、不自动晋升。 (user-visible)
- **leo-ppt-generator：接入 U4 原子输入 generation 与唯一生成入口**：新增 `generate --request` 转发 `PipelineRequest`；compose 必须消费冻结 selection，禁止空页与重新路由。pack、双 lane selection/design/binding 和 asset pins 经 staging、不可变代及 current 后才登记 RunIndex；reader 只读取已提交代，image 缺 Provider 导出时保留 blocked。 (user-visible)
- **leo-ppt-generator：收敛 U3 双层摘要与执行身份**：删除生产单一绑定摘要及旧 policy 降级读取，表达身份与 lane/主题/资产隔离；新增 canonical/proposal 判别式执行配对身份与收据引用校验。旧字段、缺字段和混用拒绝为 binding_schema_mismatch，历史 run 需重新编译。 (user-visible)
- **leo-ppt-generator：按关系和 lane 核验 U2 能力证据**：冻结独立关系 oracle；资格核对当前 owner、依赖、环境、正反例、PNG 与视觉裁决的字节引用，拒绝跨 lane 复用、证据漂移与失败反例。无视觉裁决保持 provisional，无证据保持 unverified；这些机制检查不代表真实 Provider 或全量视觉通过。 (user-visible)
...
- **leo-ppt-generator：恢复表达优先自主收口基线**：记录全部既有 dirty path 的保护归属，核验旧账本 891 项来源；651 项与归档 hash 一致、8 项可从 Git 精确恢复，75 项原位变化保留不覆盖。修复 preview 缓存复用时整册内容摘要未刷新的问题，去除 preview 单摘要字段；恢复 catalog CLI 单行 JSON 合同并校准现行容量、schema 与角色派生回归。整体 U1–U13 仍未完成。 (user-visible)
...
- **leo-ppt-generator：表达优先模板重构方案五轮全量复审收口**：基于 HEAD `7f45612b` 完成 runtime/scripts/tests/template-library/docs 全量覆盖与 Python/JSON 解析；修正非法 `review-required` readiness、锁定 124 个 `axis→axis-guide` 身份迁移与消费者切换、增加 `plan-control/declared_plan_hits` 防止 closure 自命中、补齐 focus/reading-order/`expression_incomplete` 测试合同，并清理方案重复条目。评审记录见 `docs/leo-ppt-generator/reviews/2026-09-11-expression-first-five-round-review.md`；migration/quality 独立 worker 因 429 未返回，独立性如实标记 partial，代码迁移与视觉验收仍未完成。 (user-visible)
- **leo-ppt-generator：U13 调度恢复残留修复（债2/in-flight/跨域）**：旧 operation 重放不再裸 KeyError——image 与 editable 两条 record 链路在页结果字段被 reset 清除后返回可解释 `operation_state_lost`（editable 为读取任何结果文件前的快速守卫，正常重放走原 fingerprint 路径）；`reset_failed_pages` 新增 `domain` 作用域与 in-flight 保护（目标域存在 active worker 时拒绝复位并报 `reset_blocked_by_inflight_workers`），`image sweep` 只作用 image 域、不再越界复位 editable；WS6 调度纪律观察哨接入 `dispatch_discipline_warnings` 只读聚合并入 `run diagnose`；执行合同 Worker 节补派发指引（max_concurrent_workers、vendor_revision_conflict 整命令重跑）与清扫互斥口径（R-81 残留①②）。lease TTL/续期（债1）为登记残留未实施。新增 8 项 `test_observation_recovery.py` 回归。 (user-visible)
- **leo-ppt-generator：表达优先模板重构方案五轮审查收紧**：根据当前 resolver、projection、catalog builder 与迁移脚本复核，补充损坏 catalog 与 canonical rebuild 的 fail-closed 边界、禁止隐式 point fallback、composition/template/layout 身份分离、builder 输入摘要与同代不可变、docs 消费者闭合、structure-v2 账本唯一来源及用户差页回放门；004 方案 readiness 调整为 `review-required`，未执行代码迁移或视觉验收。 (user-visible)
- **leo-ppt-generator：R-74 lane 成本路由机制**：新增 `lane_cost_comparison` 资格集合内 lane 成本对比读数——只消费 `backend_stats.jsonl` 真实账单（不乘 attempts），缺记录报 unknown 不回退假设带，`visual_weight=high` 氛围页受保护不出改道建议，冻结 backend 恒保持、真实改道只走 runtime 修订通道；读数与语义排序主键结构正交。`estimate_run_cost` 输出携带 `caliber: cost-caliber-v2/estimate-band` 口径标签并新增对账节（观测 tokens vs 估算带逐桶对照，缺观测 unknown）；`image record --backend` 去掉 `"fixture"` 缺省改为必填，`backend report` 修复 0 token 被误标 not-recorded 的显示缺陷。去饱和红例（clamp 折叠不得抹平 raw 序）等 9 项回归新增；独立标注 lane 一致率 ≥70% 属真实数据验收，仍未运行。 (user-visible)
- **leo-ppt-generator：R-73 OCR 对齐通道与 blocked 三口径统一**：新增 `image ocr` 一条龙（paddle text blocks → `page_<N>.txt`，缺 token/依赖/超时按页 not_run 披露，绝不发起半次调用）与 `image record` 前置对齐门——只对 provenance 判定的图像 lane 整页生成页生效，composite/render 豁免不做重复校验；阈值可配、豁免按条登记，WARN 期失败只披露，红例全拦且误报 ≤5% 的校准通过前拒绝 enforce 硬门；离线 fixture 校准集（干净 60/红 30 校准/验收分集）随条交付，不冒充真实 OCR。修复债8：editable status 把 blocked 计入 pending 并建议派发的三口径分叉（status/next/控制台投影）统一为独立终态，恢复只走 reset_failed_pages。visual-qa 逐字对齐职责移交机械门（语义判读保留人工）。真实 OCR 校准采集、真实误报率与成本披露汇总仍未运行。 (user-visible)
- **leo-ppt-generator：R-70a/b 组合页合成管线（阶段实现）**：新增 `render composite`（背景层 + 主题化文字层 → 终页/透明文字层/`composite_provenance` 双 provenance sidecar 三件套）与 `render composite-verify`（重推导手改拒绝）；白名单/锚点/主题化/确定性换行/对比度门机制收敛到共享 `render/text_layer.py`，TF-2 `overlay_text.py` 改为同一机制消费入口（`--theme`/`color_role`/`max_width`/`--text-layer` 新能力，TF-2 既有行为回归全绿）；带 run 身份合成时消费 `verify_design_freshness`（债4）并核对冻结绑定主题，质量观察以 `generation_method="composite"` 与 TF-2 事件分开记录。新增 20 项 `test_composite_pipeline.py` 回归（含三布局×两主题矩阵与真浏览器 receipt 链）；修复 `test_overflow_sentinel.py` 在渲染专项门命令下的既有收集错误。`page_type_regime` 路由转正（R-70c）、真实配对视觉协议与真实成本口径仍 blocked/not_run。 (user-visible)
- **leo-ppt-generator：修复 image sweep 空轮消耗预算**：无可复位页面时返回 `render_sweep_noop`，不写入轮次日志或消耗重试轮次；新增回归覆盖。 (user-visible)
- **leo-ppt-generator：TF-2 白名单与预览缓存校验修复**：overlay 保留 `required_text` 原始字符并正确应用 `--color`；预览缓存复用时校验并刷新 sidecar 的当前绑定摘要，同时保留首次渲染身份，避免像素缓存与绑定信息脱节。R-70 真实 TF/成本及视觉阶段门仍未通过。 (user-visible)
- **leo-ppt-generator：确认表达优先并补充开发期模板库重构方案**：更新 `docs/plans/2026-09-11-004-feat-leo-ppt-expression-first-plan.md`，新增语义/视觉/执行/组合目录、资产映射、新协议与消费者同步替换、一次性迁移和旧实现清理。按用户“尚未对外发布，无需向下兼容”取消双版本 reader、旧路径/身份桥和旧 run 恢复设计；保留新链的确定性构建、绑定与真实导出验收。原 Product Contract 字节保留，新增范围与兼容排除单列，R-85 表达和视觉门槛不降低。仅更新方案，未执行迁移或验证视觉改善。 (user-visible)
- **leo-ppt-generator：R-85 有效绑定继续实施**：固定选中页面与 style/theme/layout/template/font 实际字节，模板 HTML 和字体一并进入 run 快照；物化、渲染、record 检查内容与绑定摘要，收据固定快照来源。局部回归与两页真实离线渲染通过，资产全库迁移、组合页/图表与全量验收仍未完成。 (user-visible)
- **leo-ppt-generator：R-85 资产盘点入口**：新增 `--library-inventory` 只读清单，固定 canonical manifest 与唯一身份分母，显式报告别名冲突、坏 manifest、缺主题/renderer/template 和未验证组合；盘点不会把旧资产冒充为 executable。 (user-visible)
- **leo-ppt-generator：R-71 预览编排骨架**：新增 `content preview`，固定输出 `<run>/previews/` 响应式单文件总览；共享 browser/字体服务、每页独立 context、阻断外部请求，缓存损坏仅重渲相关页，保持收据五类指纹不变。40 页单模板冷启动实测 27.944 秒；image 骨架与移动端人工协议尚未验收。 (user-visible)
- **leo-ppt-generator：能力提升 R-77/R-85 持续实施**：补齐质量窗口启动/事件写入/封存 CLI、显式 TF 与真实 render 采集、固定本地渲染回放；新增内容包 v2 的章节、来源、required_text 和时长合同，旧 v1 继续可读；轻量推荐与整册池共用排名，修正二次排序覆盖语义偏好和无解误报预算耗尽。费用完整性、有效绑定全链与全量效果验收仍未完成。 (user-visible)
- **leo-ppt-generator：能力提升 R-77 度量底座实施中**：新增质量事件 schema、八字段指标注册表及只读记分卡，backend report 复用聚合器；区分 TF、缺失观测、调用费用及返工，排除构造事件、分列开发费用并保留金额精度。修复累计 tokens 重复乘 attempts 和旧 ledger 混合页号崩溃，观察哨统一字符串页号及 schema_version。真实采集、完整窗口及真实运行验收尚未完成。 (user-visible)
- **leo-ppt-generator：能力提升 PRD v1.8.1 全量需求批准**：记录用户“全部签发”，纳入 13 项活跃需求及既定验收目标，保留条件门和暂缓/关闭/移除状态；补齐需求—验收追踪与批准记录。签发不代表代码、真实视觉或收益验收。 (user-visible)
- **leo-ppt-generator：能力提升 PRD v1.8 修订为内容驱动的风格库重建方案**：新增 R-85a 首批闭环与 R-85b 全库迁移，明确章节/单页内容合同、整册主风格、资产分类与资格、唯一绑定、30 格首批覆盖及旧版本恢复；统一成本/TF/返工口径，修正回放外推、推荐覆盖率和记分卡指纹污染，更新依赖、属主及验收。仅完成需求文档优化，未实施风格迁移或重新签发。 (user-visible)
- **leo-ppt-generator：能力提升 PRD 发布门复核通过并签发（v1.6.1 基线冻结）**：发布门复核 v1.6——阻断级 T1~T5 5/5 关闭（含源码复核 regime/page_intent/suggest_layout/lint 面/unittest discover 计数 0 等自述事实）、高优级 T6~T18 13/13 关闭、常规级 T19~T32 13/14（T25 部分）——裁定**有条件签发**；3 项签发条件已落地（§8 删去不存在的「检查者」列、R-77 最小子集项集统一为"最小子集 ∪ R-70 先决清单"五项、口径 id `tf-caliber-v1` 入正文并收敛 `content_class` 术语），另随定稿并修 6 项（regime 版本戳措辞、全宽披露错字、治理条款二自锁关系、波 1 产能表述、A10 run 级 missing_policy、R-70a 成本门与 cost-caliber-v1 互引）。PRD 头部落基线冻结声明与签发状态；R-70~R-84（在册 13：12 活跃＋R-83 暂缓）自此为其唯一基线。 (user-visible)
- **leo-ppt-generator：能力提升 PRD 终审与 v1.6 阻断项修复**：三位终审专家（发布门/对抗终审/实现方验收）独立审查 v1.5——结论**不可签发**（2 票否决性、1 票有条件签发），44 项 findings 全部吸收为 `docs/prd/2026-09-11-leo-ppt-capability-improvement-prd.md` v1.6。终审核心发现：①**全批测量底座可空转完成**——R-77 最小子集三条验收全为单测且保留 `not_recorded` 万能通过出口（v1.6 增：须在 ≥1 真实 run 产出非 not_recorded 数值，全缺判 blocked，R-70 否决门不得放行）②**`page-type-regime-v1` 已并发落地而 PRD 零登记且状态已被证伪**（v1.6 增基线第 9 条、术语收敛 `page_type_regime`/`content_class`、R-74 状态改"已落地"、工程量 M→M−）③回放集与校准集无归属无验收（v1.6 定归属并补 §8 工程量）。其他关键修订：R-77 交付物三定（入口脚本/注册表文件/成本对账公式与口径版本）、效力降级判定式可判化（CI 含 1.0 判 not_passed）、R-70 三阶段止损＋波 1 产能上限＋§9.1 沉没成本清单、开工前冻结决策表（D-1~D-3）、盲评三个数值落定（非劣判定式/单项线/每组页数 N）、测试入口标签新增【判官-fixture】【回放集-proxy】两档、母版 schema 属主＝R-71、R-74 去饱和断言（实测语义加分被 clamp 饱和致首选与回退同分）、R-78 路径 `<run>/diffs/`、治理两条新条款（并发落地登记/条目准入复核）、术语与编号索引节、维护债5 修复方向定死。纪要 `reviews/2026-09-11-capability-prd-final-review.md`＋3 份终审报告归档 reviews/expert-reports/（final-a~c）。 (user-visible)
- **leo-ppt-generator：能力提升 PRD 源码核验与第三轮四域评审（v1.4/v1.4.1）**：按"必须解读源码、禁止抽查"要求，四路源码考古（状态机调度/渲染投影/脚本账本/测试评测，cli.py 等约 200 文件全文通读，结论全部带 file:line）＋四域专家复核（断言验证表+需求级判断+可重跑实验）。v1.4 新增实现现状基线并**关闭 R-81**（runtime 层 QA 与派发无互斥实证）；v1.4.1 吸收评审修正：R-73 先决改写为"generate 页图 OCR 探测一条龙"（原转换脚本在 generate 路线空转，高危）、R-71 补第六项缺口"批量渲染性能路径"（实测 1.26-1.30s/页、40 页 ≤60s 无余量，spike 前置开工门）、维护债修正升级（run-ledger 双写入方 schema 不兼容致 resume-suggestion 崩溃实证）、R-70a 补 overlay 主题化三小改与判官通道 spike 前置、TF-2 口径三注记与补登路线 a′（加性 sidecar 插入点精确到函数）、top2 最小修复方案与 R-71 缺省绑定共用排序、漂移清单扩至八项。三轮累计 12 专家+4 路考古、~150 findings；8 份报告归档 reviews/expert-reports/（src-1~4、review-r1~r4），纪要含裁定表与行动项。 (user-visible)
- **leo-ppt-generator：新增内容意图到风格版式的中间路由层**：建立版本化 `page-type-regime-v1` 真值源，新增确定性 `page_intent` 分析器，优先消费页面显式结构并输出内容形态、证据类型、视觉权重、文字密度、首选/回退版式与允许渲染 lane；`suggest_layout.py` 现在将语义意图与角色、容量、renderer 资格共同排序，低置信内容保持 `undecided`，避免把章节内容静默降级为通用列表。新增对比、趋势、流程、KPI、证据、表格和纯文本负例回归。(user-visible)
- **leo-ppt-generator：能力提升 PRD 第二轮方法论评审与 v1.3 条件吸收**：六位方法论专家（第一性原理/分而治之/MECE/逻辑层次/系统思维/双向钢人）独立审查 PRD v1.2，~50 项 findings 裁定吸收——本轮修复四个评审盲区：①R-70"≥90% 下降 CI 下界判定"在 20~30 基线事件下数学上近乎必败（效力反算+预登记降级口径+基线 go/no-go 否决门）②单人仓库盲评"≥2 独立人工评审"不可满足且与 R-79 标准矛盾（改双层评审：VLM 双判官主判 fixture 化+人工一票否决+对照组按内容类别分层）③数据饥饿回路（压制错误的 R-70/71/76 成功会抽干消费错误的 R-73/74/83 燃料；R-77 补"拦截即数据"派生字段）④MECE 空格（新增 R-84 图表构思质量基线补 AI 判断层；R-72 补重启条件）。结构性修订：R-70 拆分 a/b/c 三子阶段（各配阶段门含成本）、页型注册表 page_type_regime 单一真值源、R-77 升级指标注册表（收编孤儿指标+北极星"付费授权后用户可见返工轮次"）、呈现/披露合同归 R-76 属主、波次模型（波 1 五条/波 2 九条）、新增关联 PRD 与系统边界节（控制台/dashi/evals 触点+写侧事件契约）、层级纯化（L4 实施线索迁附录 A+吸收规则立档）。评审纪要与 6 份专家报告归档 docs/leo-ppt-generator/reviews/。 (user-visible)
- **leo-ppt-generator：PRD v1.2 移除 R-72 原生图表嵌入（图表方向调整）**：按用户决策不做 PowerPoint 原生 chart XML 嵌入（manifest `charts[]`/M2 保持预留未实现），图表能力改为 AI 自行结合材料内容思考生成——构思与设计归 AI 判断层（母版视觉行声明 + 图表样式规范 + 材料三级标注数据），产出走既有生成链（3a 密度路由 → render chart 确定性渲染 / 图像 lane）；活跃需求 14→13 项，R 编号保留不重排，全局目标计数、工程量分级与总览表同步更新，评估文档 §4.1 加移除标注保留历史分析。 (user-visible)
- **leo-ppt-generator：能力提升 PRD 五专家评审与 v1.1 条件吸收**：5 位独立专家（产品纪律/用户体验/技术架构/质量评测/成本商业化）盲评 PRD v1.0，47 项 findings 全部裁定吸收——核心修订：R-70 指标重建（TF-1 补登结构化字段或诚实收窄口径、基线盘点与 95% CI 下界判定、时序口径锁定）、盲评判读协议（≥2 独立评审/维度操作化/复用 adjudication 先例）、页型更名 hybrid→composite（避免与交付形态术语撞名）、R-70 合同修订面补全（"唯一封闭例外"红灯改封闭枚举、双 provenance 子记录、尺寸预算/交付披露口径）、实施排序重定（R-77 最小子集→R-71→R-70 分阶段转正，串行分 commit）、全批"用户净影响"行与测试入口四类型标注（单测/evals-proxy/run-留痕/人工-协议，proxy 不得宣称行为级证据）；R-72 加需求证据先决并裁 2 种图型、R-73 加 OCR 通道先决与样本量修正（干净页 ≥60/红例 ≥30）、R-74 ground truth 二选一与一致率 ≥70% 判定、R-76 用户语言强制项、R-83 暂缓待数据、R-81 先决核实升级为立即行动项。评审纪要与 5 份专家报告归档于 docs/leo-ppt-generator/reviews/。 (user-visible)
- **leo-ppt-generator：能力提升评估需求化为 PRD（R-70~R-83）**：评估文档 14 项建议转为正式需求条目 `docs/prd/2026-09-11-leo-ppt-capability-improvement-prd.md`——每条含目标、范围（含非目标）、可测验收标准与依赖触点：P0 混合页型转正（含视觉整体性盲评门与 TF 触发率 ≥90% 下降口径）、全册确定性预览（样张同轮、零 token、≤60s/册）；P1 原生图表嵌入、OCR 对齐硬门（校准未达标停 WARN 不转阻断）、页型路由优化器、版式重排提案（闭环断言）、决策简报（三要素 + 禁止编造数字）、运行质量记分卡；P2 六项。延续仓库 R 编号系列（brainstorms 已用至 R-69），全局验收纪律（红绿测试、合同同步、验证留痕）与条目状态表；评估文档头部加双向链接。 (user-visible)
- **leo-ppt-generator：新增能力提升评估文档（对照业界最佳实践与三层职责原则）**：基于执行流程梳理（SKILL.md 与 references）与 Gamma / Beautiful.ai / Plus AI / Copilot 的路线对照，以"流程稳定执行 · AI 发挥创造力与判断力 · 人决策推进节点"为评估基准（含分工判据、现状三层机制对照与 5 条原则差距），给出 14 项能力提升建议与优先级——P0 混合页型转正（背景图 + 前景文字确定性渲染，机械正确性下沉并释放 AI 创造预算）与全册确定性预览（样张门同轮补充零 token 全册证据，不新增确认轮次），P1 原生图表嵌入（charts[] M2）、required_text↔OCR 对齐硬门、页型路由优化器、版式重排提案、决策简报（变更—代价视图）、运行质量记分卡；逐条标注现状触点、基建缺口与落地前需核实的边界（合同语义修订、视觉整体性风险、调度现状待验证）。 (user-visible)
- **leo-ppt-generator：渲染质量修复文档对应的 pro 模板与主题链补齐**：八个已绑定模板现在共享结构化输入校验，母版 `结构数据` 可无损物化为 KPI、对照、时间线、卡片和表格；直接渲染拒绝错误对象、重复键、引文编造和未登记字段（保留历史 `page_no` 元数据兼容）。补齐模板区域与 slot 几何、字体和主题变量消费，修复封面/表格 DOM 锚点、时间线空值拼接、图表标签尺寸与多序列数值标注，并让渲染入口记录脚本错误、字体缺失和真实溢出。新增八个历史风格的 canonical theme 绑定及离线 Noto Serif SC 字体，重建 18 风格画廊金样板与 catalog。 (user-visible)
- **leo-ppt-generator：真实运行收据库回退修复（dashi 集成收尾验证）**：真实 chromium 三页交付链（record 含 provenance → finalize → 收据）暴露收据缺陷——模板根回退 canonical 库且存在冻结设计绑定时，run 外模板文件按 run 相对路径取键直接崩溃；现按模板库根锚定并以 `library/` 前缀入指纹（create/verify 键确定一致，库文件漂移可检出），附红绿回归测试。 (user-visible)
- **leo-ppt-generator：dashi 集成独立代码审查修复批**：7 名独立 reviewer（正确性/测试/可维护性/项目标准/对抗/API 合同/agent-native）产出 34 项 findings，按资格规则应用 13 项——stamp-page-ids 按 start_line 键配对修复交错母版错位（P1）、重复页标签（双附页/同 S 标签）在身份映射即拒绝（P1）、数值覆盖纳入表列/表行单元格（P1）、收据模板源 run 本地优先回退 canonical 库（与渲染搜索同源，防穿越锚定模板根，P1）、~20 个新 reason code 登记进 reason-codes.md（P1）、媒体数组按索引填充、baseline 失败清理捕获面放宽、canonical_json_bytes 收敛重复实现、layout-dispatch 移除已被 U1 取代的乘 0 语义表述；其余 21 项（CLI 入口扩展、freeze 顺序、selection 无包守卫等架构/合同类）转入 residual 跟踪。新增 9 个红绿回归/覆盖测试。 (user-visible)
- **leo-ppt-generator：状态视图与交付绑定收口（dashi 集成 U7/K7）**：收据新增 `content_binding` 摘要（内容包 content_digest/页身份、整册选择 policy/每页 layout、冻结设计摘要——自冻结输入派生可重建），绑定面漂移使收据失效；`verify` 对绑定不一致显式报 content_binding 类变更；`image prepare --layout-selection` 冻结整册选择并核对内容摘要与页覆盖；新增 `run_index.projection_view()` 只读视图（页身份→页序→版式→绑定摘要，可删除重建，不构成写状态入口）；`build_delivery_preflight` 新增 projection 门（页身份集合与覆盖不一致 FAIL、无内容包如实 not_run）。 (user-visible)
- **leo-ppt-generator：四类任务结构承载补齐（dashi 集成 U6/K6）**：母版新增 `表列:`/`表行:` 表格结构标记（列数对齐校验，spec-table 按声明物化 columns/rows，缺标记/不对齐回母版补齐），与既有 `对照侧:` 及要点数组共同覆盖四类高价值任务——指标及基准差异（spec-table 表列/表行）、趋势及事件注释（timeline steps）、决策矩阵（compare 对照侧）、流程及责任分工（body-basic bullets）；`tests/fixtures/dashi-integration/` 四组母版 fixture 验证「母版 → 内容包 → 硬资格 → 真实模板 data」全链，换主题不改业务内容与结构身份，7 模板 lint 零回归。 (user-visible)
- **leo-ppt-generator：结构声明准入与整册分配（dashi 集成 U5/K5）**：canonical layout schema 新增受约束 `structure` 声明（阅读顺序/分组/编码），首批 7 个 HTML 绑定版式补齐并过 schema/网格/registry 校验；结构指纹版本化规范化（重命名不变、结构语义变化即变、缺声明记 unknown 不获多样性加分）；新增 runtime `layout_selection.py`——完整合格候选池 + 确定性有界搜索（显式选择过资格门、禁复用仅对 reuse_friendly=false、预算耗尽与无候选分别报告、平分按稳定 ID 裁决），Top-2 仅为展示摘要；`capability_manifest --template-library` 新增 `structure_admission` 准入派生（42 版式：7 声明入自动池、35 记 unknown）。 (user-visible)
- **leo-ppt-generator：generate 冻结绑定与关联可编辑升级接通（dashi 集成 U4/K4）**：`image prepare --content-pack/--design` 将母版内容包与冻结设计 CAS 冻结进 run input 并登记 RunIndex——内容包摘要手改即拒、slides 页序对账、已 prepare 后改版强制新 run（fingerprint 冲突显式拒绝）；新增 `content pack` / `content stamp-page-ids` CLI（母版 → 内容包编译与一次性身份补齐）；upgrade baseline 关联扩展——source_binding 记录源 run/交付 SHA/内容与设计摘要快照（随 baseline 复制、源目录移走后仍可恢复），复制校验全部通过才发布、失败清理半份基线、同目标输入漂移明确冲突。 (user-visible)
- **leo-ppt-generator：共用候选预编译与执行投影（dashi 集成 U3/K2-K3）**：`templates.resolve_design_context()` 抽出候选与冻结设计共用的设计上下文快照（context_digest 入 resolved-design）；新增 runtime `content_projection.py`——中文角色映射唯一所有者（补「证据·实拍」别名），候选绑定先做角色/backend/容量/媒体/必需覆盖/数值覆盖硬资格（不合格不入选、软分不可抵消），选定后按同一绑定物化 HTML data 或 image prompt（image 预检不冒充成品保真）；template/layout schema 扩展嵌套输入形状（compare sides 恰 2 侧，1/3 侧硬拒绝，修复对照体静默不渲染缺口）与槽位 content_type/媒体能力，lint 增加数量声明交叉校验。 (user-visible)
- **leo-ppt-generator：母版稳定身份与内容包投影（dashi 集成 U2）**：新增 runtime `content_pack.py` 公共母版解析与 `page-content-pack-v1` schema——页首 `page_id: pg-<hex>` 稳定身份跨 revision 保留（插删换序不重编号，一次性写入，部分携带即损坏），confirmed 母版可编译为带内容摘要的确定性内容包（手改被拒，engineering 备注与 notes 隔离），同数不同单位/期间不合并，缺字段登记表行/图行按母版位置报错；重投影与母版校验脚本改用公共解析，来源 manifest 在素材更换（同 figure ID）后不再继承旧 hash/审查字段。 (user-visible)
- **leo-ppt-generator：启动 Dashi 集成实施**：新增覆盖 U1–U7 的实施计划与容量硬超、真实 layout schema、canonical 模板依赖漂移回归用例；完整生成链路与成品收益仍待验证。
