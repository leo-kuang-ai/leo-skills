...
仅在顶层已判定 `interaction_mode=execute` 后进入本页。`advise` 请求不得运行本页的
launcher、setup 或 config，也不得创建项目/run；只需回答路线、状态和下一步。
...
所有非纯内容状态响应先输出 `route/status/reason_code/execution_eligibility/next_action`
五字段摘要；字段缺失或互相矛盾时停止消费该状态，不自行推导下一步。
