...
Command support root: `.qoder/skills/spec-code-review`. Treat it as the loaded skill directory whenever this inlined workflow refers to `SKILL_DIR` or the directory containing `SKILL.md`.
...
1. First read `.qoder/skills/spec-code-review/references/modes-and-output.md`, parse arguments, and freeze mode, mutation, commit, and dispatch policy. `mode:agent` is always report-only.
2. At Stage 1 read `.qoder/skills/spec-code-review/references/scope.md` to resolve the diff, task attribution, and local scope snapshot. Missing dispatch authorization or capability selects its inline fallback with explicit degraded coverage and required-gate limitations.
3. At Stage 2 read `.qoder/skills/spec-code-review/references/intent-and-plan.md` to establish intent and the current plan/task completeness scope.
4. When dispatch is authorized, at Stage 3 read `.qoder/skills/spec-code-review/references/persona-catalog.md` and `.qoder/skills/spec-code-review/references/select-and-route.md` to select the roster, standards, and risk paths. Peer admission remains subject to the actual receipts, egress authorization, and provider limits in `.qoder/skills/spec-code-review/references/cross-model-review.md`.
5. At Stage 4 read `.qoder/skills/spec-code-review/references/dispatch-reviewers.md`, load the selected prompt assets, and dispatch within available capacity. Collect every started reviewer's result before merging; a pending return is not an empty finding set.
6. At Stages 5–6 read `.qoder/skills/spec-code-review/references/finish-review.md`. Run findings mechanics before semantic synthesis, then the triggered validation, mutation guard, and report output. Never treat raw reviewer artifacts as the completed report. Read `.qoder/skills/spec-code-review/references/action-class-rubric.md` for the shared severity and action-routing definitions.
