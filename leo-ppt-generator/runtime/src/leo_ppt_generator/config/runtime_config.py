...
def _normalize_document(
    ...
    *,
    ...
    if "preferred_provider" in raw:
        preferred = raw["preferred_provider"]
        if preferred not in EXTERNAL_PROVIDERS:
            raise RuntimeConfigError("provider_selection_invalid", "/preferred_provider")
    ...
    if preferred is not None and preferred not in normalized["provider_profiles"]:
        raise RuntimeConfigError("provider_selection_invalid", "/preferred_provider")
...
class ConfigStore:
    """schema v2 ConfigStore；以 canonical digest 提供进程间 CAS。"""
    ...
    def reset(self) -> RuntimeConfig:
        ...
        with FileLock(str(self.lock_path), mode=0o600):
            _atomic_write_document(self.path, document, self.environ)
            return self.read()
