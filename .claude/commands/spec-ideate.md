...
If no argument is provided, apply the subject gate in `.claude/spec-first/workflows/spec-ideate/references/scope-gates.md`; open-ended discovery requires the user's Surprise me choice.
...
1. Read `.claude/spec-first/workflows/spec-ideate/references/output-mode.md` and `.claude/spec-first/workflows/spec-ideate/references/scope-gates.md` before Phase 0, even when scope and format are already clear. Resolve output/resume, subject, mode, substance, scaling, and the cost notice before grounding.
2. Read `.claude/spec-first/workflows/spec-ideate/references/grounding.md` before Phase 1. Route named files as directive or evidence before either grounding batch; await the results and disclose failed or thin grounding.
3. Read `.claude/spec-first/workflows/spec-ideate/references/decomposition.md` before deciding whether the subject is atomic. Only Surprise me may skip this read in advance, recording its skip reason. Otherwise run Phase 1.5 after grounding and before generation.
4. Read `.claude/spec-first/workflows/spec-ideate/references/divergent-ideation.md` before software generation. For elsewhere-non-software, read `.claude/spec-first/workflows/spec-ideate/references/universal-ideation.md` instead and use its depth, frames, and critique.
5. Read `.claude/spec-first/workflows/spec-ideate/references/post-ideation-workflow.md` after generation/merge and before critique, persistence, or handoff. It owns the checked deliverable and next-step handling; rendering references load only at write time.

### Required phase ownership

- Subject, mode, substance, scaling, and cost transparency belong to `.claude/spec-first/workflows/spec-ideate/references/scope-gates.md`; output and resume to `.claude/spec-first/workflows/spec-ideate/references/output-mode.md`; topic axes and evidence scouts to `.claude/spec-first/workflows/spec-ideate/references/decomposition.md`.
- Grounding, user research artifacts, issue intelligence, learnings, and external-research authorization are owned by `.claude/spec-first/workflows/spec-ideate/references/grounding.md` and its named references.
- Software generation belongs to `.claude/spec-first/workflows/spec-ideate/references/divergent-ideation.md`; critique, persistence, and handoff to `.claude/spec-first/workflows/spec-ideate/references/post-ideation-workflow.md`. Universal generation and domain-native critique use `.claude/spec-first/workflows/spec-ideate/references/universal-ideation.md` with the shared persistence contract.
