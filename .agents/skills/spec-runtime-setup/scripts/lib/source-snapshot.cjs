...
const { detectRuntimePlatform, getEffectiveRegistry } = require('./registry.cjs');

function captureSourceSnapshot({ repoRoot, skillRoot = path.resolve(__dirname, '../..'), host, platform, homeDir, env = process.env, now = new Date() } = {}) {
  ...
  try {
    ...
    const registry = getEffectiveRegistry(JSON.parse(registryBytes), { host, platform: currentPlatform });
    const paths = new Map();
