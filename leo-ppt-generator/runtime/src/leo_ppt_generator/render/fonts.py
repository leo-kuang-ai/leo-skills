...
- ``/<template>.html|.css`` → ``assets/render-templates/``（模板与其相对引用）
...
from .assets import fonts_dir, templates_dir, vendor_dir


def _handler(root_map: dict[str, Path]):
...
class RenderAssetServer:
    ...
    def __init__(self, *, extra_font_dirs: list[Path] | None = None) -> None:
        font_roots = [fonts_dir(), *(extra_font_dirs or [])]
        existing = [root for root in font_roots if root.is_dir()] or [fonts_dir()]
        handler = _handler({"/": templates_dir(), "fonts": existing})
