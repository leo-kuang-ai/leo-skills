...
Before checkout, server discovery, startup, or browser handoff, read [Run preparation](.claude/spec-first/workflows/spec-polish/references/run.md). It owns workspace safety, startup facts, server attribution, and the verified actual URL. Keep the mutation facts above in force. Do not enter the loop until the selected server is reachable.
...
- `.claude/spec-first/workflows/spec-polish/references/run.md` - workspace, server startup, and browser handoff
...
The Bash tool's working directory is the user's project, not the skill directory, so a bare `scripts/<name>` path will not resolve — invoke each by the skill's own absolute path via `SKILL_DIR` (see `.claude/spec-first/workflows/spec-polish/references/run.md`). Scripts:
