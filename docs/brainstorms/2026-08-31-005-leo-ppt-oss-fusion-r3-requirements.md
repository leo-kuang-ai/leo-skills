...
readiness_verified_at: 2026-08-31T10:34:04.105Z
readiness_checker_schema: spec-prd-artifact-check.v1
readiness_finding_count: 1
readiness_blocking_count: 0
readiness_prd_hash: sha256:d6bd0548e90f5329f2bf060eea2e45bbc68bdfcf192f3ce4f8510e260fe78d1a
...
> 风格体系批(R-61~R-68)底座:`/Users/kuang/knowledge/ppt-github/` 设计链 9 份文档(仓库外,126 个 AI-PPT 项目两轮调研 + Style 管理模型/推荐引擎/适配性论证 v2/最优性论证,用户 2026-08-31 确认回写)。
...
| 业务域 | leo-ppt-generator 技能包(内容引擎/内容质量/文案/视觉/一致性/交付/治理/风格体系八域) |
...
| 相关文档 | 六份专家报告(source_inputs);`/Users/kuang/knowledge/file-github/分析报告/组0-7.md` 与 `调研报告.md`(仓库外);`/Users/kuang/knowledge/ppt-github/` 风格体系设计链 9 份文档(仓库外,R-61~R-68 来源);`docs/plans/2026-08-31-001/002/003`(P0/P1/P2 已落地批);`leo-ppt-generator/evals/known-issues.md` |

## Summary

以 6 位"PPT 产品专家 + 技术架构师"对 152 个开源写作/内容生态项目的机制级调研为基础,并以 ppt-github 风格体系设计链(126 个 AI-PPT 项目)补强风格域,把其中约 69 条经源码/文档验证的借鉴点转化为 leo-ppt-generator 的产品需求:把技能从"材料→PPTX 转换器"升级为"轻形态研究引导(R-01)+条件启用完整代采(R-69)的内容引擎",为母版真值补上"承诺账本 + 影响面推导"两块内容层确定性核对,建立 deck 文案/讲稿的语言质量纪律与图上文字合成协议,打通确定性导出族与风格缩略图可发现性,硬化风格推荐(硬规则/跨家族多样性/别名/金样板回归/家族合并),并用统计判据收口评测三大在案债务——全部增量不削弱任何既有治理门禁(人在回路、收据、分级、凭据边界)。
...
| add(新增) | LEO_PPT_HOME 下 library/ 素材库、decks-styles/ 蒸馏档案、export 子命令族、eval_stats 统计工具、敏感文本扫描、图上文字合成协议 reference | 与 profiles/、brands/ 同构的第三/四用户档案通道 | 新增本地数据目录:素材库经用户主动入库存储素材本体+sha256+来源元数据(仅存本地、随 LEO_PPT_HOME 数据目录管理、提供整库导出与清理入口;与档案"只存偏好字段"的边界见 R-04);反馈日志只存归类标签不存原话(见 R-64) | external-research |
...
编号规则:R-01 至 R-69(连续编号,R-69 为评审后拆分追加;域小节仅为组织视图)。优先级 P0(首批,世界级门槛)/P1(次批)/P2(择优后置)。来源列:域 A~G 为专家N-编号(source_inputs 专家报告);域 H 为 ppt-github 设计链文档名。

### 域 A:内容引擎(输入端,R-01~R-05、R-69)

| 编号 | 触发条件 | 角色 | 系统行为(EARS:应…) | 用户可见结果 | 来源 | 优先级 |
| --- | --- | --- | --- | --- | --- | --- |
| R-01 | 用户无自备材料 | 系统 | 执行**研究代采轻形态**:产出研究问题清单(该 deck 需要回答的问题,按大纲骨架组织)与建议检索渠道/素材类型清单,经合同确认门确认后交付用户自行取材;不执行任何联网采集(轻形态零宿主依赖) | 从"被 blocked 要材料"变为"获得一份可执行的研究问题清单与取材指引";用户取材后回流按 generate 既有流程;需要 leo 代为采集时走 R-69(条件启用) | 专家1-#1 的清单化降维;评审后按 owner 确认由完整代采降为轻形态(2026-08-31) | P0 |
...
| R-04 | 用户把图片/图表/数据表/引用存入素材库(LEO_PPT_HOME/library/) | 系统 | 入库属用户主动动作,存储素材本体并以 sha256 + 来源元数据 + 标签登记;库仅存本地、随 LEO_PPT_HOME 数据目录管理(不入技能同步面)、提供整库导出与清理入口;母版视觉行引用库内素材时自动携带出处,sources-manifest 从库登记派生 | logo/产品照/历史图表跨 run 复用且出处自动完备;库外编造链接仍被 strict 校验拒绝;与交付档案的边界:档案只存偏好字段,素材库存用户主动入库的素材本体 | 专家5-E5-01(Beav 目录式素材库) | P1 |
...
| R-69 | 用户在 R-01 轻形态基础上明示要求代为采集,且宿主具备联网检索、且轻形态使用信号已验证需求真实 | 系统 | 执行完整研究代采:先确认研究问题清单,再采集,把每条 learning 与来源 URL 绑定落盘为材料包(content/research-pack.md,逐条可回指),然后按 generate 既有流程消化;采集内容受 BR-006 第三方内容权利边界约束 | "内容引擎"完整形态;未验证/无能力/未授权时维持轻形态(R-01)与 input_material_missing 既有语义(降级先例:Provider 三态/worker 缺失固定块) | 专家1-#1(gpt-researcher learnings 原子引用);由原 R-01 完整代采拆出,启用条件=轻形态验证(2026-08-31 owner 确认) | P1 |
...
| 编号 | 触发条件 | 角色 | 系统行为(EARS:应…) | 用户可见结果 | 来源 | 优先级 |
| --- | --- | --- | --- | --- | --- | --- |
| R-24 | generate 路线产出全出血封面/大图井页面 | 系统 | 执行图上文字合成协议:安静区约束注入(母版视觉行事前声明文字落位区与主体映射——标注人脸/主体位置,文字只进安全区——并注入生成提示词,属构图决策而非事后测试)→无蒙版优先→失败才允许局部图像色调蒙版(限定标题区、取图内色调、峰值透明度约束)→缩略图终检归既有视觉 QA 轮(多模态判定层,BR-004 语义层例外) | 图像 lane 高废片页型获得前置决策协议而非事后补丁;与确定性优先规则不冲突 | 专家3-V1(guizang image-overlay 四步阶梯) | P0 |
...
| 编号 | 触发条件 | 角色 | 系统行为(EARS:应…) | 用户可见结果 | 来源 | 优先级 |
| --- | --- | --- | --- | --- | --- | --- |
| R-47 | 用户要求 handout-PDF / 长图 / 轮播卡片等衍生形态 | 系统 | 提供 export 子命令族:全部走 M1 渲染 lane 确定性渲染,导出物并入交付收据指纹,导出属交付动作、经 DELIVERY-GATE 披露;handout-PDF 经渲染 lane 逐页 PNG 加确定性合成产出(字体已随渲染栅格化,不引入新排版引擎);首批范围 handout-PDF+长图,carousel 接入既有社交卡片规格(解除实验性状态)随 R3-4 择优 | 一份 deck 多形态交付;确定性渲染保证同输入同输出 | 专家5-E5-07(elog 多目标导出架构;P1 搁置理由已消除);范围按 owner 确认裁剪(2026-08-31) | P0 |
...
| 编号 | 触发条件 | 角色 | 系统行为(EARS:应…) | 用户可见结果 | 来源 | 优先级 |
| --- | --- | --- | --- | --- | --- | --- |
| R-51 | 多轮评测报告存在 | 系统 | 提供评测统计工具:输入多轮结果(带输入格式版本探测,格式不识别时如实报错不静默),输出每 case PASS 率置信区间(Wilson 95% CI)、判官修复前后重放序列显著性检验(Mann-Whitney U/Fisher),置信区间跨 0.5 或样本不足标 trending;bench"至少 3 轮采样"口径升级为置信区间判据 | M1 17 例判官校准/词表债/80-case 轮归因三大债的收敛判据去主观化;统计显著不自动晋升 judge(人在回路修复协议保留) | 专家6-E6-1(ai-marketing-skills 统计检验引擎) | P0 |
...
| R-53 | 材料含未公开/涉密样貌数据、分级确认前 | 系统 | 运行确定性敏感文本扫描(模式:未脱敏手机号/证件号/用户词表内部代号),输出候选页+命中位置+遮蔽串(保留前后各 1-2 字符,不回显完整敏感值,防扫描结果二次暴露)供 agent 语境复核;按分级门控规则集;候选不等于结论(两层检测) | 分级确认获得"哪里值得看"的机读证据;误杀由语境层把关 | 专家6-E6-3(yuwen 两层检测) | P1 |
...
### 域 H:风格体系与推荐引擎(R-61~R-68)

> 来源:ppt-github 风格体系设计链(仓库外 9 份文档)。该链最优性论证选定的"五层资产+编译面+治理面"混合架构与 leo 现状(style render 字节红线=编译一致性、四条治理 lint=治理面、brief=spec、版式库=L2、brands/=L3)方向一致;本域只补 leo 真缺的拼图,不重构既有体系。

| 编号 | 触发条件 | 角色 | 系统行为(EARS:应…) | 用户可见结果 | 来源 | 优先级 |
| --- | --- | --- | --- | --- | --- | --- |
| R-61 | 风格推荐执行时 | 系统 | 先经确定性硬规则层(15-25 条 YAML:场景体裁排除/锁定家族,如答辩排除赛博朋克系、党政语境锁定红系、儿童受众排除高攻击性家族),再进入语义推荐;硬规则可被用户显式点名否决(bypass) | 低级风格错配从"靠 LLM 自觉"变为"结构性不可能";规则零成本毫秒执行 | 《Style推荐引擎设计》第三节(硬规则层) | P1 |
| R-62 | 风格推荐呈现 2-3 个方向时 | 系统 | 候选池送精排前按家族配额保证跨家族多样性(每家族至多 2 条),最终推荐方向必跨家族;首方向明确标注推荐 | "三个深蓝推荐等于没推荐"被结构性消除;方向间差异可比较 | 《Style推荐引擎设计》第四节(多样性硬约束) | P1 |
| R-63 | 风格 brief 定义/合并时 | 系统 | brief 增 aliases 字段(中英别名与俗称,如 mckinsey/麦肯锡风/深蓝商务),推荐匹配走别名模糊命中;家族合并时保留全部来源原名入 aliases | 用户口语化风格名("麦肯锡风")直接命中库内风格,无需知道库内 slug;oh-my-ppt 74/74 别名字段实测为其真正资产 | 《Style管理模型》L1 schema;《适配性论证》事实 1 | P1 |
| R-64 | 每次风格推荐落盘时 | 系统 | 记录 brief 信号归类/候选/推荐/用户选择/否定归类;反馈仅存本地、否定只落风格家族或风格的降权标签(用户原话仅当轮会话内使用,不落盘),保留随 LEO_PPT_HOME 数据目录管理并提供清理入口;用户措辞补进 aliases 候选须先经治理确认 | 推荐越用越准,用户否定有记忆而非每轮重来;与档案"绝不存业务数据"口径一致 | 《Style推荐引擎设计》第六节(反馈闭环) | P2 |
| R-65 | 风格 brief 或渲染链变更后 | 系统 | 每套具备确定性渲染路径的风格(11 套内置及后续 token 化的合并主风格;prompt-only 参考风格无渲染回归基准,豁免)保留三页金样板(封面/内容/图表,由 M1 渲染 lane 确定性生成),变更后与金样板做像素/结构 diff,漂移超阈值报警并阻断该版本晋升 | 字节红线管一致性之外,视觉上限获得回归基准("编译器管一致性,金样板管上限");与 R-26 缩略图同资产双用途 | 《Style管理模型》治理面第 7 条(金样板回归);《最优性论证》短板 1 | P1 |
| R-66 | 风格库存量治理时 | 系统 | 按家族把 126 条参考风格去重合并为主风格+variants(裁决规则:有实际令牌者为主、元数据全者为辅),来源原名全数保留;治理 lint 增家族计数校验防回潮;合并完成时存量计数口径(README 数字/风格索引计数断言/画廊条目/lint 基线白名单)一次性联动迁移,以四条治理 lint 全绿为完成判据 | 137 条库存收敛为"有效风格+变体",推荐选择面与索引健康度提升 | 《适配性论证》第六节(14 对同源 slug 合并测试用例)、第七节(337 条收敛 60-100) | P1 |
| R-67 | 高频场景风格推荐时 | 系统 | 提供场景预组合预设(风格+版式集+叙事+密度+fallback 链);学术垂直既有隐性预设显式化;预设缺失时按 fallback 降级而非硬失败 | 董事会汇报/答辩/路演等场景的推荐负担与跨 deck 一致性双受益 | 《Style管理模型》L4 预设层 | P2 |
| R-68 | 图像 lane 风格执行时 | 系统 | brief 增插画配对字段(插画家族+浓度档 core/supportive/sparse),slide worker 按配对保持全稿插画家族与浓度一致 | 插画气质页间漂移(色字版式锁定覆盖不到的维度)被风格合同锁定 | ppt-master paired-rendering;《适配性论证》吸收增强 2 | P1 |
...
- **BR-006(第三方内容权利边界)**:BR-005 的上游授权仅覆盖对开源项目机制的借鉴,不延伸至运行时采集的第三方网络内容。研究代采(R-01)与数据通道(R-05)采集的内容只存元数据与引用锚点(来源 URL+抓取时间);引用文字进页面走三级标注既有合同;图源素材须来自可商用来源或经用户权利确认后才可入页;交付披露含采集内容清单。

## 优先级分级

| 编号 | 优先级 | 可降级方案 | 是否阻塞"世界级门槛" |
| --- | --- | --- | --- |
| R-01 | P0 | 不可降级;轻形态零宿主依赖(完整代采见 R-69,条件启用) | 是 |
| R-15 | P0 | 检测规则集可分期,框架不可降 | 是 |
| R-16 | P0 | 同 R-15 | 是 |
| R-24 | P0 | 协议可先覆盖封面+大图井两类版式 | 是 |
| R-34 | P0 | 不可降级 | 是 |
| R-35 | P0 | 不可降级 | 是 |
| R-47 | P0 | 已裁剪:首批 handout-PDF+长图,carousel 随 R3-4 择优 | 是 |
| R-51 | P0 | 不可降级(三大债收口依赖,兼作 M1.1 收口判据) | 是 |
| R-03/R-04/R-06~R-10/R-17~R-19/R-25~R-28/R-36~R-41/R-48/R-49/R-52~R-55/R-61~R-63/R-65/R-66/R-68/R-69 | P1 | 域内可再排序 | 否(但合计决定次批价值) |
| R-02/R-05/R-11~R-14/R-20~R-23/R-29~R-33/R-42~R-46/R-50/R-56~R-60/R-64/R-67 | P2 | 可整体延后或择优 | 否 |

## Scope Boundaries

### 本期做

- 上述 8 域 69 条需求的全量 WHAT 定义、验收与分期建议(执行分批落地,见 Feature Slices;P2 维持择优口径,owner 已确认)。
...
- **不做风格 Embedding 粗排与版式字符预算移植**:leo"索引+按需读单文件"已是更省的漏斗形态(材料自己的 n≥30 分档判据下 leo 形态更优);vw 容量模型+密度档位已比 char_budget 更细(ppt-github 设计链结论)。
- **不做七阶段管线主干重构**:leo 合同→大纲→母版→样张+回退链已是七阶段的等价实现,重排是伪需求(《高质量PPT生成-七阶段拆解》机会区印证 leo 现架构)。
- **不做 Beamer/LaTeX 编译目标**:leo 边界明示不做,适配性论证同因归参考库(《适配性论证》修正 4)。
...
| --- | --- | --- |
| AE-01 | R-01 | Given 用户无自备材料;When 进入 generate 首轮;Then 呈现研究问题清单与建议检索渠道/素材类型并请求确认(含剩余确认序列预告),不直接 blocked;不执行任何联网采集(轻形态零宿主依赖) |
| AE-02 | R-01 | Given 用户仍无材料且未满足 R-69 条件;When 用户要求"帮我直接找材料";Then 维持 input_material_missing 固定块语义并指回研究问题清单自取材,不静默模拟研究结果 |
...
| AE-16 | R-53 | Given 材料正文含 11 位手机号且尚未分级;When 敏感文本扫描执行;Then 输出候选页、命中位置与遮蔽串(不回显完整敏感值)供分级复核;候选不直接触发拒做 |
...
| AE-62 | R-47 | Given 用户要求轮播卡片导出;When export carousel(随 R3-4 择优批次);Then 按既有社交卡片三画板规格产出,规格脱离"实验性"标注 |
| AE-71 | R-69 | Given 轻形态使用信号已验证、宿主具备联网检索且用户明示要求代采;When 完整代采执行;Then 逐条 learning 与来源 URL 绑定落盘 research-pack 且可回指;图源经 BR-006 权利校验;未满足任一条件时回复走 R-01 轻形态 |
| AE-63 | R-61 | Given 学术答辩场景请求风格推荐;When 推荐执行;Then 赛博朋克/蒸汽波等家族被硬规则前置排除(不依赖 LLM 自觉);用户显式点名某被排除风格时规则让位并在 style 合同记录用户选择依据 |
| AE-64 | R-62 | Given 候选池含 5 个深蓝商务系风格;When 推荐呈现;Then 家族配额先行,2-3 个方向必跨家族,首方向带推荐标注与归因 |
| AE-65 | R-63 | Given 用户说"麦肯锡风"而库内 slug 为其他命名;When 风格匹配;Then 经别名命中对应主风格;合并场景下各来源原名保留于 aliases 可检索 |
| AE-66 | R-64 | Given 用户否定某推荐方向并称"太严肃";When 下次同场景推荐;Then 该风格家族带降权标记参与排序,否定措辞进入 aliases 候选(经治理确认后生效) |
| AE-67 | R-65 | Given 风格 brief 修订导致渲染输出视觉漂移;When 金样板回归执行;Then 三页金样板 diff 超阈值报警并阻断该版本晋升;缩略图与金样板同源生成 |
| AE-68 | R-66 | Given 库内存在多个同族参考风格(历史多来源收录);When 家族合并执行;Then 产出主风格+variants,来源原名全数保留,治理 lint 家族计数通过;重复收录回潮被 lint 拦截 |
| AE-69 | R-67 | Given 董事会汇报场景;When 预组合命中;Then 推荐直接呈现风格+版式集+叙事+密度的组合与 fallback 说明;预组合资产缺失时按 fallback 降级而非硬失败 |
| AE-70 | R-68 | Given 图像 lane 全稿逐页生成;When 风格执行;Then 各页插画家族与浓度按 brief 配对字段一致,换页不换插画气质 |

## Goals / Success Metrics

| 目标类型 | 目标描述 | 衡量口径 | 数据来源 |
| --- | --- | --- | --- |
| 产品目标 | 材料缺失场景从 100% blocked 到轻形态研究引导(完整代采 R-69 依使用信号验证) | R-01 落地后新 case 在线绿;advise/execute 回复含研究问题清单分支知识;R-69 启用前不要求联网能力 | evals 在线轮 |
...
| 质量目标 | 风格推荐硬化生效 | R3-2.5 后:硬规则零低级错配(对抗样本)、推荐方向 100% 跨家族;首推接受率/前三命中率记录首轮基线后观察 | 风格推荐 case + 反馈记录 |
...
| 上游许可不构成阻断 | user-stated | 用户任务书("线下与作者沟通授权");该授权仅覆盖上游项目借鉴,不延伸至运行时采集的第三方内容(后者由 BR-006 约束) | 已确认 |
...
| 风格体系批(R-61~R-68)机制依据 | external-research | ppt-github 设计链 9 份文档(仓库外,含 v2 适配性论证);该链自声明局限:约 44% 项目覆盖、生成者-验证者同源、盲测未做 | 实施前按 leo 自有 137 brief 逐项核验,不照搬 28 项目分级;设计链文档仍在用户侧迭代以最新版为准 |
...
| requirements_acceptance | filled | - | 69 R / 71 AE,全覆盖 | - | - |
...
| data_compliance_security | filled | - | R-53/R-48/凭据边界/BR-006;素材与反馈数据边界见 R-04/R-64 | - | - |
...
| 硬规则层 | 风格推荐前的确定性家族排除/锁定规则(YAML,可被用户点名否决) | ppt-github 设计链 |
| 金样板回归 | 每风格三页人工认可的渲染基准,变更后 diff 防视觉上限漂移 | ppt-github 设计链 |
| 家族合并 | 同族多来源风格收敛为主风格+variants,来源原名入 aliases | ppt-github 设计链 |
...
| 风险 | 影响 | 缓解 / 接受 |
| --- | --- | --- |
| 68 条范围过大导致落地失焦 | 分批拖长、半成品面增多 | Feature Slices 分五批+每批独立可验证(沿用 P0-P2 批纪律);P2 允许择优 |
...
| 全量开发期间评测配额与限流(429 在案) | 批次验收被环境失败拖慢 | 批次间预留评测窗口;超时不计行为信号(known-issues 既有纪律);R3-1 拆两子批分摊校准压力 |
...
| 安全 | 敏感扫描本地执行且输出遮蔽命中值;素材库/反馈日志的本地存储与清理边界见 R-04/R-64;采集内容权利边界见 BR-006;导出不泄露隐藏元数据 | 单测+审计 | 是 |
...
| R-61 | BR-004 | AE-63 | ppt-github《Style推荐引擎设计》第三节 | P1 |
| R-65 | BR-004 | AE-67 | ppt-github《Style管理模型》治理面第 7 条 | P1 |

## Feature Slices

<!-- prd:section=feature_slices -->
### 分期建议(owner 已确认执行序,2026-08-31;批次内文件级拆解属 spec-plan)

- **R3-0 前置批(开发前置,非新能力)**:R-51 评测统计工具 + M1.1 收口(入口锚点行修复、17 例判官按 M0.1 协议重放校准、金样 HEX 2 例裁决、全量轮复绿)。R-51 是 M1.1 收口的判据工具,先行实施;此批完成前不启动 R3-1。
- **R3-1a 门面批·账本与文案(4 条)**:R-15 文案检测框架、R-16 讲稿纪律、R-34 承诺账本、R-35 影响面;随批 BR-001 入口锚点+judge 校准。
- **R3-1b 门面批·视觉与导出(2 条)**:R-24 图上文字协议、R-47 导出族(handout-PDF+长图;carousel 随 R3-4)。拆批理由:单批不超历史最大批(4 项)一倍以上,评测校准压力分轮释放(M1 团灭的结构性诱因)。
- **R3-2 内容批(P1)**:R-06~R-10、R-17~R-19、R-03/R-04、R-53/R-55。
- **R3-2.5 风格体系批(P1,排在 R3-2 内容批之后、R3-3 状态与视觉批之前)**:R-61 硬规则、R-62 跨家族多样性、R-63 别名、R-65 金样板回归(与 R-26 同资产)、R-66 家族合并、R-68 插画配对;实施顺序建议 硬规则/多样性→金样板→家族合并(先治存量再扩能力);R-64 反馈闭环与 R-67 场景预设随 R3-4 择优。
- **R3-3 状态与视觉批(P1)**:R-36~R-41、R-25~R-28、R-48/R-49、R-52、R-54、R-69(若轻形态信号已验证)。
- **R3-4 择优批(P2)**:按使用信号择优(含 R-47 carousel、R-64、R-67、R-56 等挂起项),其余留接缝声明;P2 整体维持择优口径,不承诺全量(owner 已确认)。

## Planning Recheck

| item | why recheck | required before | blocks planning? |
| --- | --- | --- | --- |
| **M1.1 收口状态核验**(17 例判官校准/入口锚点修复/金样 HEX 2 例) | 在案债务不清零即叠新能力=M1 团灭模式复现;R-51 为其判据工具 | R3-1a 动工(R3-0 批内完成) | 是(R3-0 批出口条件) |
...
| leo 137 brief 的家族分布实测 | R-66 合并方案需按 leo 实际库存核验(设计链 28 项目分级不可照搬) | R3-2.5 动工 | 否 |
| R-01 轻形态使用信号(代采需求占比) | R-69 完整代采的启用前提是需求被验证 | R-69 实施前 | 否 |
| 学术海报形态(PosterCraft 等 2026 趋势) | 技术栈与 slides 趋同但形态新,R3 未立项 | R3-2.5 落地后按学术用户信号复评 | 否 |
| product-lens/scope-guardian 补审(评审 limitations 在案) | 68 条范围与 P0 门槛主张在该两视角射程内,标准档未跑;范围决策已由拆批/轻形态/择优处置,如需独立复核可 roster:full 补一轮 | spec-plan 前(可选) | 否 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| OQ-1 | 分期顺序是否照 Feature Slices 执行 | Feature Slices | answered(2026-08-31 owner 确认"全部问题按最佳方案修复") | no | owner-answered | no | closed | 照 R3-0→R3-1a→R3-1b→R3-2→R3-2.5→R3-3→R3-4 执行;批次内文件级拆解属 plan |
| OQ-2 | R-01 研究代采默认启停与形态 | R-01/R-69 | answered(2026-08-31 owner 确认) | no | owner-answered | no | closed | 轻形态(R-01,零宿主依赖)默认启用;完整代采(R-69)条件启用:轻形态使用信号验证+宿主联网+用户明示要求三者齐备;降级先例见 SKILL.md 不变边界 |
| OQ-3 | R-56 跨技能共享工件是否启用 | R-56 | answered(2026-08-31 owner 确认) | no | owner-answered | no | closed | 默认不启用,随 R3-4 择优;启用前须在两技能 SKILL 与 AGENTS.md 显式登记该共享契约 |
...
| ppt-github 风格体系设计链结论回写(2026-08-31 会话:"OK") | user-stated | 新增域 H 八条(R-61~R-68)入 R3 PRD,排 R3-2.5 批;不做 Embedding/char_budget/七阶段重构/Beamer 编译(入 Non-Goals) | 域 H/Feature Slices/Non-Goals | 风格体系增量与 R3 既有批次同 PRD 管理,执行仍按分期 | closed |
| OQ-1:全部功能进入开发前的分期执行序(2026-08-31 会话:"全部问题按最佳方案修复") | user-stated | 照 R3-0(前置收口)→R3-1a→R3-1b→R3-2→R3-2.5→R3-3→R3-4 执行;P2 维持择优不承诺全量;R3-1 拆两子批控制评测校准压力 | Feature Slices/优先级分级 | M1.1 收口完成前不启动 R3-1 新能力批 | closed |
| OQ-2:研究代采形态与启停 | user-stated | R-01 降为轻形态(研究问题清单+取材指引,零宿主依赖)默认启用;完整代采拆为 R-69(P1)条件启用:轻形态使用信号验证+宿主联网+用户明示要求 | R-01/R-69/AE-01/02/71 | 唯一扩产品边界项的风险敞口后置到需求验证之后 | closed |
| OQ-3:R-56 跨技能共享工件启停 | user-stated | 默认不启用,随 R3-4 择优;启用前须两技能文档显式登记 | R-56/Feature Slices | 避免未验证收益的双技能登记面 | closed |
...
| 2026-08-31 | ZCode(spec-prd refine) | refine:吸收 ppt-github 风格体系设计链(126 项目)结论,新增域 H 八条(R-61~R-68)、AE-63~70、R3-2.5 风格体系批;Non-Goals 补不做 Embedding 粗排/char_budget/七阶段重构/Beamer 编译;Planning Recheck 补 137 brief 家族分布实测与学术海报观察项 |
| 2026-08-31 | ZCode(spec-prd refine + doc-review) | 评审修复(spec-doc-review 三视角 9 项发现全量落地):素材库存储口径矛盾修正(R-04/Change Delta/NFR 三处对齐);新增 BR-006 第三方内容权利边界并接入 R-01/R-05/Evidence;R-24 安全区改"约束注入"消除与 BR-004 确定性冲突;R-64 反馈日志补本地存储/只存归类标签/清理入口边界;R3-3.5 更名 R3-2.5 并写死批次顺序(R3-2 后、R3-3 前);R-66 补存量计数口径迁移判据;R-47 补 PDF 经 PNG 确定性合成无新排版依赖;R-53/AE-16 补遮蔽串防二次暴露;R-65 金样板明确覆盖确定性渲染路径风格(prompt-only 豁免) |
| 2026-08-31 | ZCode(spec-prd refine,owner 确认"全部问题按最佳方案修复") | 开发前待确认点全量落地:R-01 降为轻形态(研究问题清单+取材指引,零宿主依赖),完整代采拆为 R-69(P1,轻形态信号验证后条件启用);R-47 裁剪(carousel 随 R3-4 择优);Feature Slices 重构为 R3-0 前置批(R-51+M1.1 收口,R3-1 出口条件)+R3-1a/1b 两子批;P2 维持择优口径;R-56 默认不启用;OQ-1/2/3 全部 owner-answered 并绑定追溯行;Planning Recheck 补 M1.1 收口核验(阻塞项)/轻形态使用信号/可选补审三行;风险表补评测配额与限流缓解;R-51 补输入格式版本探测 |

## Handoff

- 本 PRD 语义闭合(P0 全验收覆盖、降级语义明确、Non-Goals 硬边界、开发前待确认点全量落地),可进入 spec-plan 做批次化实施计划(建议从 R3-0 前置批起步:R-51 评测统计+M1.1 收口)。
