...
1. 冻结内容合同：主题、受众、使用场景、演讲时长、行动目标、页数、素材来源、
   ...
   `strongest_objection`(最可能推翻 `one_thing` 的最强反方及其证据锚点)与
   `invalidation_triggers`(`one_thing` 失效的可观测触发条件,带阈值或事件);
   材料确无反方证据时写「无已知反方+检索依据」,不得为空;靠隐藏反证获得的
   结论按内容事实层失败处理,母版须在收束页前以页面角色「边界」或
   `argument_role`「反方」承载(见 deck-master.md)。**金额测算纪律**:行动目标
   与收束页请求中的任何金额(预算/分期/gate 余额/分配额)须满足其一——登记表
   含测算行(拆到可核参数或引用报价),或显式 `unknown`+补齐时限与 owner;
   裸估算金额不得进入请求页。**护栏最小完备**:收束页与认错/失效线的每个
   动作项必须携带 owner 与时限,验证类动作必须附交付物定义(可核实的输出
   物——报告/数据表/签字文件);「持续加强」「保持关注」类无交付物表述
   不得充当动作项(`check_master_contract` 的⑪⑫ WARN 判据为其机器子集)。
   **给定结论冲突处置**:用户结论与材料证据冲突时,
   结论按证据适用域限定继承、不擅改;冲突以显式页呈现;出路三选(修改结论/
   补充未见证据/书面留痕),留痕类动作优先会前单独沟通,不当众处置。数字与因果断言按**四级标注**：
