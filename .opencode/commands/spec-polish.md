...
Command support root: `.opencode/skills/spec-polish`. Treat it as the loaded skill directory whenever this inlined workflow refers to `SKILL_DIR` or the directory containing `SKILL.md`.
...
Before checkout, server discovery, startup, or browser handoff, read [Run preparation](.opencode/skills/spec-polish/references/run.md). It owns workspace safety, startup facts, server attribution, and the verified actual URL. Keep the mutation facts above in force. Do not enter the loop until the selected server is reachable.
...
- `.opencode/skills/spec-polish/references/launch-json-schema.md` — launch.json schema + per-framework stubs
- `.opencode/skills/spec-polish/references/run.md` - workspace, server startup, and browser handoff
- `.opencode/skills/spec-polish/references/dev-server-detection.md` — port resolution documentation
- `.opencode/skills/spec-polish/references/dev-server-rails.md` — Rails dev-server defaults
- `.opencode/skills/spec-polish/references/dev-server-next.md` — Next.js dev-server defaults
- `.opencode/skills/spec-polish/references/dev-server-vite.md` — Vite dev-server defaults
- `.opencode/skills/spec-polish/references/dev-server-nuxt.md` — Nuxt dev-server defaults
- `.opencode/skills/spec-polish/references/dev-server-astro.md` — Astro dev-server defaults
- `.opencode/skills/spec-polish/references/dev-server-remix.md` — Remix dev-server defaults
- `.opencode/skills/spec-polish/references/dev-server-sveltekit.md` — SvelteKit dev-server defaults
- `.opencode/skills/spec-polish/references/dev-server-procfile.md` — Procfile-based dev-server defaults

The Bash tool's working directory is the user's project, not the skill directory, so a bare `scripts/<name>` path will not resolve — invoke each by the skill's own absolute path via `SKILL_DIR` (see `.opencode/skills/spec-polish/references/run.md`). Scripts:
- `.opencode/skills/spec-polish/scripts/read-launch-json.sh` — launch.json reader
- `.opencode/skills/spec-polish/scripts/detect-project-type.sh` — project-type classifier
- `.opencode/skills/spec-polish/scripts/resolve-package-manager.sh` — lockfile-based package-manager resolver
- `.opencode/skills/spec-polish/scripts/resolve-port.sh` — port resolution cascade
