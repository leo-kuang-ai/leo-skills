...
10. **Offer an optional next-work handoff, then finish.** Apply `references/shipping-tail.md` after the current pipeline reaches its terminal state. Do not invoke `spec-handoff` before the user explicitly accepts. Output `<promise>DONE</promise>` only after required verification, durable residual handling, and an empty needs-human decision set; otherwise render and return every complete decision without claiming completion. Disclose the actual terminal state and limitations.
