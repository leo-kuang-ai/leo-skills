...
## 11. UX 交互合同（2026-08-28 v4 新增）

### 首次成功路径（Onboarding）

- 新用户空状态：两个入口卡（"写一篇文章 / 做一份演示"）+ 示例项目一键体验（预置已跑完的管线，直接看证据账本与门禁留痕——aha 前置）。
- 新建项目向导三步：选工位 → route card 表单（四轴 + 证据模式档位 + depth 分段控制）→ 确认创建；顶部步骤指示，可回退；第 2 步即实时预览管线将经过的阶段。
- 裸主题低置信：向导内嵌"决定性问题"单选，未答前创建按钮禁用并说明原因（把 skill 的拒绝语义翻译成引导，而非报错）。

### 状态矩阵（每类界面必须覆盖五态）

| 界面 | empty | loading | active | awaiting/blocked | failed |
|---|---|---|---|---|---|
| Dashboard | 空态引导卡 | 骨架瓦片 | 任务运行卡 | 门禁橙点 + blocked 卡 | 失败卡（保留已完成进度条） |
| 写作工位 | 引导语 + 示例段落 | 阶段脉冲 | 编辑器四色标注 | 内联门禁卡 | 阶段失败横幅 + 续跑按钮 |
| PPT 工位 | "选 Route 开始" | 样张骨架格 | 页面瀑布 | 样张门禁 / Gate 0 阻断卡 | reason code + 已产样张保留 |

- **failed 的 UX 合同**：必须展示已完成阶段的产物可查看（"失败有回礼"），提供断点续跑入口，reason 用人话 + canonical code 双层呈现。
- **blocked 的 UX 合同**：原样透传 reason_code；唯一出路按钮 = 显式确认动作；确认文案注明"将留痕"。

### 门禁疲劳对策（渐进披露）

- 门禁分级：**快速门禁**（单选、默认展开、支持 ⌘⏎ 提交）与**审阅门禁**（需看内容，收起为卡片流，点开局部展开）。
- "稍后处理"：每个门禁卡提供延迟决策选项——任务保持 awaiting_user、用户可离开、导航橙点提醒；不静默替用户决定。
- 密度规则：同屏最多一个展开的门禁卡；新门禁到达时旧的自动折叠为留痕行。

### 反馈与动效补充

- 所有决策动作 300ms 内给反馈（toast 或状态条变化），长任务每阶段切换必须有可见脉冲——"没死，在工作"。
- 数字滚动仅用于证据计数与用量；列表入场 stagger ≤8 项。
- 危险动作（删除账户/项目）双段确认：第一段说明后果，第二段要求输入名称或密码。
