...
请前往 [红狐hub](https://redfox.hk/settings/api-keys) 获取API KEY
...
| 环境变量 | 说明 | 是否必填 | 获取方式 |
| -------- | ---- | -------- | -------- |
| `REDFOX_API_KEY` | 红狐数据 API Key | 是 | [红狐hub](https://redfox.hk/settings/api-keys) |
...
**Q: 如何获取 API Key？**
A: 请访问 [红狐hub](https://redfox.hk/settings/api-keys) 注册并获取 API Key，按本文"一键安装"章节配置环境变量。
