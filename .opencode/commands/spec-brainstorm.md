...
Command support root: `.opencode/skills/spec-brainstorm`. Treat it as the loaded skill directory whenever this inlined workflow refers to `SKILL_DIR` or the directory containing `SKILL.md`.
...
| --- | --- | --- |
| Before the first question, including non-software work | `.opencode/skills/spec-brainstorm/references/interaction-rules.md` | Core principles, source lookup before questions, single current-user confirmation, question shape, and output economy. |
| Before scope questions or classifying carried decisions | `.opencode/skills/spec-brainstorm/references/settled-decisions.md` | Settlement, directives, and the Product Contract decision owner. |
| Phase 0.0, before other phases | `.opencode/skills/spec-brainstorm/references/output-mode.md` | Exclusive format, config precedence, token parsing, and pipeline override. |
| Phases 0.1-0.3 | `.opencode/skills/spec-brainstorm/references/phase-0.md` | Resume, domain and verdict routing, scope, visual decisions, and unfamiliarity. |
| Phase 1 | `.opencode/skills/spec-brainstorm/references/dialogue.md` | Current-tree grounding, private scratch, source-sensitive questions, decision-relevant vocabulary/code conflicts, pressure test, and dialogue exit. |
| Phases 2-2.6 | `.opencode/skills/spec-brainstorm/references/approaches.md`; also `.opencode/skills/spec-brainstorm/references/synthesis-summary.md` before synthesis | Approaches, scenario coverage, confirmation, and claim verification. |
| Phase 3 | `.opencode/skills/spec-brainstorm/references/plan-write.md`, then `.opencode/skills/spec-brainstorm/references/brainstorm-sections.md` and the selected rendering reference | Document need, Product Contract, durable recovery, and promotion candidates. |
| Phase 4 | `.opencode/skills/spec-brainstorm/references/handoff.md` | Scope-aware options, format, downstream payload, and closing summary. |

Before asking scope questions, read `.opencode/skills/spec-brainstorm/references/settled-decisions.md`. Carry settled product decisions forward without re-asking them; spend at most one in-pipeline challenge on an unexamined directive. The Product Contract's Key Decision entry is the single owner of each accepted product decision: annotate it there and reference it elsewhere instead of repeating it.
...
- Before declaring a document written or handing it off, apply the checks owned by `.opencode/skills/spec-brainstorm/references/brainstorm-sections.md`. Preserve source refs, limitations, blockers, and the next question across pauses; scratch is not durable handoff evidence.
