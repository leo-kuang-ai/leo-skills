...
- `advise`：用户只要求判断、解释、比较路线、汇报状态或说明下一步，或明确说“先不要
  ...
  `references/styles/00_索引/_INDEX.md` 及其指向的 `references/styles/generated/`
  下 Markdown 名称范围页、分面摘要和 counts，按需读取，不加载 catalog.json 或完整 brief。
...
- 名称/别名页只索引 style/pool，不是论证模式或 P 码版式全集。用户问论证模式或版式规则时，直接使用本入口摘要；未知枚举留到 execute 核对对应轴文档，不在风格索引里反复找，也不把未命中说成该模式或版式不存在。
  学术五拍属于论证模式，执行期权威源是 `references/styles/06_论证模式/学术五拍.md`；advise 说明由 `style render --mode 学术五拍` 注入，结论句标题按论证模式条件化，结论用具体数字而非形容词。五拍精确顺序延迟到 execute 核对原文，不凭名称编造。
- advise 检索工具必须限定具体 `.md` 文件，或在 generated 下显式设置 `glob=*.md`；不得无后缀约束搜索整个 generated，因为其中含全量 catalog JSON。读错范围后不能通过“回答不依赖这些行”豁免。
  点名查询可用宿主 `Grep(pattern="目标名|别名", path="<skill>/references/styles/generated", glob="names-*.md", output_mode="content")`，再 Read 命中分页；等价只读终端查询也必须限制相同文件范围。宿主没有任何合规读取能力时报告无法查询。
- advise 的目录证据仅是随包快照，不能声称已核对用户 home。generated 缺失、明显损坏或已知过期时，
  不读坏生成段、不运行脚本、不凭记忆列风格；说明“当前无法确认风格目录”，唯一下一步为“恢复有效索引后继续查询”。
- execute 进入视觉方向阶段后，使用当前 runtime Python 运行 `scripts/capability_manifest.py --style-index --check`
  做只读检查。该输出只证明索引新鲜，不证明视觉效果。失败时不重建安装目录，直接使用独立源摘要查询。
...
- 强视觉复用遵循既有 sidecar 的 `max_per_deck`；P9 Closing Manifesto 每个 deck 最多一页，不能连续两页使用。定稿前由 `scripts/check_layout_reuse.py` 复核，用户点名不绕过结构上限。
...
- 新版式提交到 `references/styles/12_版式库/` 必须同时提供同名 `.layouts.json` sidecar，并通过 `scripts/lint_layout_grid.py` 与 `scripts/lint_style_governance.py`。六字段 Markdown 不能代替 sidecar，也不能先入库后补门禁；个人未登记草稿应留在库外。advise 可直接说明此规则，不需读取额外 reference。
...
- **协作方式（CONFIRM-GATE）**：执行请求默认委托执行，按已有授权自主完成可逆的
  合同、大纲、母版、风格和样张制作、审查及范围内修复。用户明确选择逐阶段共创时，
  在约定里程碑呈现具体工件后等待；合同与大纲、视觉方向与样张可同轮呈现，修订展示差异。
  内部内容、来源、容量与样张质量检查始终保留。“确认/批准”在后续 reference 中指完成
  对应决策节点，不自动意味着每件工件都要等待人工；真实工具的人工状态条件另行遵守。
...
- **模版推荐与选择**：风格候选由合同信号驱动、默认推荐必须带归因一句；用户指定
  ...
  确认门（见 `references/style-recommendation.md`）。委托执行可由 Agent 完成样张审查并据实记录决策来源；不能跳过真实出图、读回和质量检查。generate 路线的大纲与逐页母版必须在 `<project-root>/content/` 下落盘（`outline-v<N>.md` / `deck-master-v<N>.md`），头部携带 `confirmation` 和决策依据，聊天只引用路径与变更摘要；该文档门仅在 execute 模式生效。
