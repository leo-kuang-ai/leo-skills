...
Command support root: `.claude/spec-first/workflows/spec-write-skill`. Treat it as the loaded skill directory whenever this inlined workflow refers to `SKILL_DIR` or the directory containing `SKILL.md`.
...
| Owner blocked | create/revise 的 owner 不唯一、跨 repo、repo-external、generated-only 或 containment 未确认。保留 `base_operation=create|revise` 与 `effect=apply`，结果为 `blocked-source-owner`。 | 读 [Authoring Method](.claude/spec-first/workflows/spec-write-skill/references/authoring-method.md)，给 candidate-only preview、空 would-change/command list 和唯一下一步。 | 未绑定单一 canonical source 时零 mutation；不得降格为 `not-entered` 或猜测 owner。 |
| Validate-only | 用户明确检查现有/外部 package；现有 package 使用 `base_operation=revise` + `effect=validate-only`，结果为 `portable-readiness-report` 或 `trust-preflight-blocked`。 | no-follow inventory、bundled validator 与 [Delivery Gates](.claude/spec-first/workflows/spec-write-skill/references/delivery-gates.md)。 | 报告后停止；不得执行目标 scripts、validator、hooks、binaries 或 lifecycle，不得跟随 symlink、读 secret-like 内容、复制、安装或写入。 |
| Tier A apply | 已确认 owner 的 behavior-preserving revise，具体条件由 workbench 定义。 | 读 [Authoring Workbench](.claude/spec-first/workflows/spec-write-skill/references/authoring-workbench.md)，确认当前授权覆盖 exact write set、preview binding 与最窄结构验证。 | receipt/验证后 close out；承重行为变化转 full apply。 |
| Full apply | 已确认 owner 的 create/revise apply，结果为 `portable-core-only`、`portable-core-with-behavior-contract` 或 `spec-first-project-profile`。 | 依次读 [Authoring Method](.claude/spec-first/workflows/spec-write-skill/references/authoring-method.md)、[Authoring Workbench](.claude/spec-first/workflows/spec-write-skill/references/authoring-workbench.md) 与 [Shape-Aware Evaluation Design](.claude/spec-first/workflows/spec-write-skill/references/evaluation-design.md)；写 core 前完成紧凑 Design Record 与最小 pre-patch eval plan。Capability Map、显式 shape/module decision 或 topology 只在它们改变 owner、consumer、resource/runtime carrier、架构或风险时展开。 | preview、授权、风险匹配验证和 source update 后 close out；缺 baseline/eval plan 时停止，缺 semantic/comparative evidence 时降级对应 claim。 |
...
| --- | --- | --- | --- |
| 判定 workflow、owner/effect 或 portable core | [Authoring Method](.claude/spec-first/workflows/spec-write-skill/references/authoring-method.md) | qualification、resolution、resource ownership | near-neighbor 路由；owner 不明时 preview-only / blocked。 |
| Tier A 或 full apply | [Authoring Workbench](.claude/spec-first/workflows/spec-write-skill/references/authoring-workbench.md) | Tier A，或紧凑 Design Record、按风险展开的 map/topology、preview handoff | 未完成必要 design record/preview 不得 apply。 |
| prose、persona、few-shot、输出合同或 agentic loop 承重 | [Behavior Contract Design](.claude/spec-first/workflows/spec-write-skill/references/behavior-contract-design.md) | 行为 delta、authority、examples、checkpoint/stop | 纯工具/schema 不读；不以 persona prose 代替行为合同。 |
| Full apply 需要 baseline、protected behavior 或 eval family | [Shape-Aware Evaluation Design](.claude/spec-first/workflows/spec-write-skill/references/evaluation-design.md) | 最小 pre-patch eval plan 与风险匹配的 evidence family | 未读不得开始 full apply source patch；Tier A 不触发。 |
| measurable optimization 或 field feedback 改变 disposition | [Optimization And Feedback Handoff](.claude/spec-first/workflows/spec-write-skill/references/optimization-and-lifecycle.md) | optimization handoff、feedback-to-regression | optimization 主意图路由 `spec-optimize`；未复现 feedback 保持 observation。 |
| package/写前/closeout 验证或风险信号 | [Delivery Gates](.claude/spec-first/workflows/spec-write-skill/references/delivery-gates.md) | validator、preview、risk checks、五轴 readiness | 记录 `not-run` / degraded，不伪造 gate。 |
| target metadata/invocation 或本地治理/catalog/generator 改变 patch | [Target Profiles](.claude/spec-first/workflows/spec-write-skill/references/target-profiles.md) 或 [Project Profiles](.claude/spec-first/workflows/spec-write-skill/references/project-profiles.md) | target/project facts 和 projection | portable-only 不读 profile；owner 不明不 apply。 |
