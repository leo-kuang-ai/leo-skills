...
Deterministic CLI: `{{LEO_PPT}}`. This is the absolute executable path supplied by the parent from the runtime_manager ensure output (`cli_reference`); use it for every deterministic command. Any bare `leo-ppt` text in fixed schema fields is a command-family label, not a PATH executable.
