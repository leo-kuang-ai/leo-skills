...
Command support root: `.claude/spec-first/workflows/spec-debug`. Treat it as the loaded skill directory whenever this inlined workflow refers to `SKILL_DIR` or the directory containing `SKILL.md`.
...
`<bug_description>` and load `.claude/spec-first/workflows/spec-debug/references/pipeline-return.md`. This mode is for
...
Read `.claude/spec-first/workflows/spec-debug/references/investigate.md` now for issue provenance, the dirty-tree comparison, and investigation safeguards before gathering evidence. Phase 2's causal-chain and authorization gates remain in this entrypoint.
...
- **Does not reproduce after 2-3 attempts:** Read `.claude/spec-first/workflows/spec-debug/references/investigation-techniques.md` for intermittent-bug techniques.
...
- If the bug looks like a regression ("it worked before"), use `git bisect` (see `.claude/spec-first/workflows/spec-debug/references/investigation-techniques.md`)
...
Follow the prior-work search in `.claude/spec-first/workflows/spec-debug/references/investigate.md`; carry related tickets and failed approaches into the findings without creating a new issue of record.
...
Follow the assumption audit and hypothesis procedure in `.claude/spec-first/workflows/spec-debug/references/investigate.md`, including its anti-pattern read and predictions for uncertain links.
...
Read `.claude/spec-first/workflows/spec-debug/references/fix.md` before writing Phase 2's recommendation. Its regression procedure applies only to confirmed defects, never to a deliberate behavior reversal.
...
`.claude/spec-first/workflows/spec-debug/references/pipeline-return.md`: apply a convergent local fix only when the
...
Use `.claude/spec-first/workflows/spec-debug/references/fix.md` for failed-fix invalidation, escalation, defense-in-depth, and post-mortem triggers.
...
return from `.claude/spec-first/workflows/spec-debug/references/pipeline-return.md`. Do not run a nested shipping tail,
