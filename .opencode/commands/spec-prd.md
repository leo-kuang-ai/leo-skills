---
description: "Run the Spec-First PRD requirements workflow"
argument-hint: "[increment request, existing PRD path, or validation target]"
---

Command support root: `.opencode/skills/spec-prd`. Treat it as the loaded skill directory whenever this inlined workflow refers to `SKILL_DIR` or the directory containing `SKILL.md`.
...
- `.opencode/skills/spec-prd/references/evidence-and-topology.md` - current-state evidence tags, Change Delta, source-candidate boundaries, Framing Gate, topology, surface, producer/consumer, source-of-truth, contradiction, and negative-space rules.
- `.opencode/skills/spec-prd/references/product-analysis-lite.md` - load only when Phase 0 sees the exact `analysis_profile=contract-reset-lite` token. Use its single Product Analysis Brief as the compatibility representation of Requirement Analysis Gate + Product Expert risk ranking + Push-Right checkpoint; keep legacy artifact/finalize behavior and load deeper references only when the Brief exposes their trigger.
- `.opencode/skills/spec-prd/references/domain-language-and-decision-ledger.md` plus optional `docs/contracts/domain-glossary.md` - terminology, domain boundaries, source/user/glossary contradictions, bounded grill, Pre-PRD Clarification Loop, Deep Requirements Grill, Context / ADR Topology Adapter, and decision notes.
- `.opencode/skills/spec-prd/references/grill-with-docs-integration.md` - package-local historical snapshot plus the adapted candidate-only contract: sustained one-question-at-a-time interview, source-first lookup, glossary challenge, PRD-local closure, and project-level promotion candidates without project mutation. Load by default for PRD authoring/refinement from rough PRD, draft, `reference-claims`, `resume-prd`, `pure-text`, or multi-source material unless the request is wrong-stage, implementation-ready, or already fully source-resolved.
- `.opencode/skills/spec-prd/references/product-expert-lens.md` - default authoring hot path: downstream-confirmation risk ranking, Product Expert Lens interface, structured-input synthesis, design-source/large-input pointers, and escalation boundary.
- `.opencode/skills/spec-prd/references/design-source-evidence.md` - trigger-only for front-end/UI inputs with design links, screenshots, exported design context, or interaction-state material; design facts stay advisory until source/owner reconciliation.
- `.opencode/skills/spec-prd/references/large-input-checkpoint.md` - trigger-only for oversized, multi-source, long-chain, or resume-risk PRDs; reduced candidates feed Product Expert Lens and PRD sections act as checkpoints.
- `.opencode/skills/spec-prd/references/prd-output-template.md` - machine-safe output contract、section identity、Product Expert Lens write-in、PRD quality diagnosis、P0/P1 quality packs、template composition order 与 project-local overlay 边界；不再承载 template routing map 或重复的 human-facing 正文骨架。
- `.opencode/skills/spec-prd/references/prd-readiness-lens.md` - final PRD quality, Pre-PRD Clarification closure, triggered P0/P1 pack closure, readiness, handoff, or doc-review decision.

## Template Trigger Map

模板是 npm 产品内置运行资产，按需组合，不能每次全量读取：

1. 每个会产出 PRD artifact 的 run 都读取 `.opencode/skills/spec-prd/assets/templates/00-generic.md`。
2. 根据 `target_surface` 只读取一个 primary surface 模板；只有真实 Mixed 需求才增加必要的 secondary surface 模板。
3. 只有当前执行对话用户确认拆分边界后，才读取 `.opencode/skills/spec-prd/assets/templates/70-large-requirement-index.md`。
4. 只有输入、项目 source 或当前执行对话用户明确证券/交易信号时，才读取 `.opencode/skills/spec-prd/assets/overlays/securities.md`；无行业信号时不得加载。
5. 用户项目自己的模板、术语和行业规则按 project-local overlay 读取相关片段，不复制到 product-bundled assets。

| target_surface / trigger | 读取资产 |
| --- | --- |
| every PRD artifact | `.opencode/skills/spec-prd/assets/templates/00-generic.md` |
| App | `.opencode/skills/spec-prd/assets/templates/10-app.md` |
| Admin | `.opencode/skills/spec-prd/assets/templates/20-admin.md` |
| Backend / Java | `.opencode/skills/spec-prd/assets/templates/30-backend.md` |
| H5 / PC | `.opencode/skills/spec-prd/assets/templates/40-h5-pc.md` |
| CLI / DevTool / workflow / runtime | `.opencode/skills/spec-prd/assets/templates/50-cli-devtool.md` |
| real cross-surface or producer/consumer change | `.opencode/skills/spec-prd/assets/templates/60-mixed.md` |
| owner-confirmed oversized split | `.opencode/skills/spec-prd/assets/templates/70-large-requirement-index.md` |
| securities / trading signal | `.opencode/skills/spec-prd/assets/overlays/securities.md` |

`.opencode/skills/spec-prd/references/prd-output-template.md` 先提供 frontmatter、machine-owned section、readiness、trace 与 finalize 合同，再组合上述正文资产。Human-facing 模板不得预填 `status: ready-for-planning`、`readiness_verified_*` 或 ready receipt。
...
Choose `output_shape` before drafting, apply the machine-safe contract from `prd-output-template.md`, then compose `.opencode/skills/spec-prd/assets/templates/00-generic.md`, one primary surface template, and only the triggered built-in/project-local overlays. Include conditional sections only when they reduce planning invention; do not copy run-local scratch into the PRD by default.
