...
Command support root: `.qoder/skills/spec-runtime-setup`. Treat it as the loaded skill directory whenever this inlined workflow refers to `SKILL_DIR` or the directory containing `SKILL.md`.
...
Before any project-local write, read `.qoder/skills/spec-runtime-setup/references/project-config.md`. Project-local config actions never install providers or edit host config; host/provider actions never migrate local config keys.
...
- helper/provider actions：仅在显式选择并授权的 mutation mode 中安装或验证依赖；裸调用仅执行只读诊断，不安装工具、不构图、不修改 host config 或 setup facts。
...
Runtime Setup 按当前 mode 的副作用边界执行；裸调用仅执行只读诊断。
...
当前唯一公开入口是 `spec-runtime-setup`（Claude/Qoder 命令拼写 `runtime-setup`）；不提供 `spec-mcp-setup` / `mcp-setup` 兼容别名。调用它的 host runtime surface 是权威 host evidence。Generated host-specific runtime surface 必须在调用支持 mutation 的 Node mode 前，通过 per-call environment 固定 `MCP_SETUP_HOST=<host>`。缺少显式 canonical `MCP_SETUP_HOST=claude|codex|cursor|kiro|opencode|qoder|zcode` 时，`setup.cjs` 必须 fail closed；不得根据 `PATH`、generated runtime 目录、旧 `.spec-first/config/*` facts 或其他平台的 host config 文件推断 mutation target。只读诊断可以展示 advisory host candidate，但这些 candidate 不具备 write authority。
...
When this generated Qoder `spec-runtime-setup` runtime surface invokes `.qoder/skills/spec-runtime-setup/scripts/*`, set `MCP_SETUP_HOST=qoder` in the script environment. Do not rely on automatic host detection from PATH, because Claude Code, Codex, and Qoder CLIs can coexist on the same machine.
...
- Bare invocation (`spec-runtime-setup`)：裸调用仅执行只读诊断，复用 Node bare mode；配置冲突只报告，不自动修复，不将诊断成功表述为完整 setup ready。
...
2. 执行 `node "$SKILL_DIR/scripts/setup.cjs"`，保留调用方的 target flags。裸调用仅执行只读诊断；不刷新 example、不创建 override、不写 ignore、facts 或 host config，也不安装 Provider。
...
4. 用户要求安装或修复时，使用相同 target 和 selection 的 `--plan` 预览；已授权 scope 可继续显式 mutation mode。未授权的新副作用需要确认；预览成功本身不授予写入权限。
...
Registry v11 使用 `readiness_policy=always-required|workflow-required|advisory`。`--only <ids>` 显式选择优先，`--workflow <ids>` 仅匹配 `required_for` 中的精确 ID；不从自然语言、文件存在或用途描述猜测需求。Facts item 的 `demand` 记录 blocking、demand_source、matched_rule、reason_code 和 migration_warning。旧 v10 缺少 policy 时保留旧阻断语义并提示迁移。未请求的工具缺失不阻断无关 scope；显式请求的实际能力失败仍阻断该 scope。
...
- 仅通过已授权的 `--repair-host-config` 替换冲突的 managed MCP entry；裸调用不授予修复权限，保留无关 host config，高优先级或不安全目标仍阻断；
