...
For each candidate, execute the flow that matches its classification from Phase 2 (confirmed in Phase 3). Read `.claude/spec-first/workflows/spec-compound-refresh/references/per-action-flows.md` and follow the matching section:
...
**First, read `.claude/spec-first/workflows/spec-compound-refresh/references/concepts-vocabulary.md`.** This is unconditional. Do not pre-judge from memory which Phase 1 signals qualify — the reference's criteria are non-obvious and a "nothing qualifies" judgment without reading is a shortcut, not a result.
